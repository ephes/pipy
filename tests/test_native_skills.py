...
def test_find_skill_by_name_is_case_sensitive(tmp_path: Path) -> None:
    workspace = _make_workspace(tmp_path)
    skills_dir = workspace / ".pipy" / "skills"
    _write_skill(
        skills_dir,
    ...
    skills, _ = _discover(workspace)
    assert find_skill_by_name(skills, "Lint") is not None
    assert find_skill_by_name(skills, "lint") is None
