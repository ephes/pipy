"""Golden wire-byte fixtures for the OpenAI Chat Completions adapter.

These characterization fixtures pin the exact request the
``native.providers.openai_completions`` adapter serializes onto ``native.http``
(the Chat Completions ``messages`` serialization, the ``tools`` shape, and the
mapped ``reasoning_effort``), the parsed success usage/output, and the sanitized
...
_FIXTURES = Path(__file__).parent / "fixtures" / "openai_completions"
...
def test_openai_completions_request_matches_golden_wire_bytes(tmp_path: Path) -> None:
...
def test_openai_completions_success_response_parses_from_golden(tmp_path: Path) -> None:
    client = RecordingJsonHTTPClient(
        JsonResponse(status_code=200, body=_load("response_success.json"))
    )
    provider = OpenAIChatCompletionsProvider(
...
def test_openai_completions_error_metadata_matches_golden(tmp_path: Path) -> None:
    ...
    provider = OpenAIChatCompletionsProvider(
        model_id="gpt-4o-mini", api_key="sk-test", http_client=client
    )
