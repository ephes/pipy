"""Golden wire-byte fixtures for the OpenRouter Chat Completions adapter.

These characterization fixtures pin the exact request the
``native.providers.openrouter`` adapter serializes onto ``native.http`` (the Chat
Completions ``messages`` serialization, the ``stream: false`` flag, and the flat
``tools`` shape), the parsed success usage/output (including OpenRouter's
``cached_tokens``/``reasoning_tokens`` detail fields), and the sanitized
HTTP-status error metadata. They exist so the Phase 5.2 provider-family move
cannot silently alter the bytes on the wire: the request body is captured
...
_FIXTURES = Path(__file__).parent / "fixtures" / "openrouter"
...
def test_openrouter_request_matches_golden_wire_bytes(tmp_path: Path) -> None:
    ...
    # Structural parity: chat messages serialization, stream flag, and tools.
...
def test_openrouter_success_response_parses_from_golden(tmp_path: Path) -> None:
    client = RecordingJsonHTTPClient(
        JsonResponse(status_code=200, body=_load("response_success.json"))
    )
    provider = OpenRouterChatCompletionsProvider(
...
def test_openrouter_error_metadata_matches_golden(tmp_path: Path) -> None:
    ...
    provider = OpenRouterChatCompletionsProvider(
        model_id="openai/gpt-4o", api_key="sk-test", http_client=client
    )
