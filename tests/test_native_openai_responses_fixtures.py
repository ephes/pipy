"""Golden wire-byte fixtures for the OpenAI Responses adapter.

These characterization fixtures pin the exact request the
``native.providers.openai_responses`` adapter serializes onto
``native.http`` (canonical-message ``input`` serialization, ``tools`` shape,
and Responses-native ``reasoning.effort``), the parsed success response, and
the sanitized error metadata. They exist so the Phase 5.2 provider-family
move cannot silently alter the bytes on the wire: the request body is captured
straight off a recording ``RecordingJsonHTTPClient`` and compared to a checked-in
golden, both structurally and as the exact serialized wire payload.
"""
...
_FIXTURES = Path(__file__).parent / "fixtures" / "openai_responses"
...
def test_openai_responses_request_matches_golden_wire_bytes(tmp_path: Path) -> None:
    ...
    # Structural parity: input serialization, tools, and reasoning.effort.
...
def test_openai_responses_success_response_parses_from_golden(tmp_path: Path) -> None:
    client = RecordingJsonHTTPClient(
        JsonResponse(status_code=200, body=_load("response_success.json"))
    )
    provider = OpenAIResponsesProvider(
...
def test_openai_responses_error_metadata_matches_golden(tmp_path: Path) -> None:
    ...
    provider = OpenAIResponsesProvider(
        model_id="gpt-5.1-codex", api_key="sk-test", http_client=client
    )
