...
`name` and `description`. The body of the file is the literal message
text the dispatcher sends when the user invokes `/<name>` at the REPL
prompt.
...
`pipy_harness.native.workspace_context` and reuses the shared resource
loader in `pipy_harness.native._resource_files`. No body content is
...
class CustomSlashCommand:
    ...
    Markdown text the dispatcher sends as the user message when the
    command is invoked; it may be empty for a frontmatter-only file.
    `sha256` and `byte_length` always describe the file as it exists on
    disk; `truncated=True` means the body in this object only contains
    the first per-file-cap bytes plus a deterministic marker.
    """
