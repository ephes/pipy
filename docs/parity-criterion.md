...
| Extension/package platform | 🟡 Pi-shaped core, not Pi-equivalent | Runtime skills/templates/custom commands/themes ship. The Python extension API now has discovery/activation, commands, shortcuts, lifecycle/input/prompt hooks, `tool_call` gates, extension tools, `tool_result` transforms, minimal `ctx.ui.notify`, golden conformance, provider-registration mechanics, a local-path package CLI, package runtime composition, and per-run source-loading flags: installed local-path packages contribute extensions/skills/prompts/themes through discovery at lowest precedence with `+/-pattern` filters, and explicit CLI paths can load those resource kinds while default discovery is disabled. Remaining Pi gaps: rich UI/rendering/session hooks, dynamic tool/model/thinking controls, `user_bash`/provider-payload hooks, state/session-manager helpers, remote packages, update, and supply-chain policy. Source: `docs/extension-api.md`. |
