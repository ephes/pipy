...
| Product TUI/editor workflow | 🟡 partial | Inline TUI, slash menu, `/settings`, `/model`, history, paste, undo/redo, resize, `/copy`, `/scoped-models`/Ctrl+P cycling, queued steering/follow-up basics, and true provider-request cancellation ship. Still missing the `@` file picker (Pi uses exact/prefix/substring scoring, not fuzzy), richer path completion, clipboard/drag image paste, `!`/`!!`, thinking hotkeys/folding, richer overlays/selectors, mouse selection, and long-input soft wrapping. Specified in `docs/tui-workflow.md`. |
...
| Provider/model catalog | 🟢 catalog + completions-family product construction, gated | A pipy-owned built-in catalog (`native/catalog.py`/`catalog_data.py`), the layered matcher (`native/model_resolver.py`), the `models.json` custom-provider/override loader with routing/compat deep-merge and `refresh`/`register_provider` (`native/models_json.py`), thinking (`native/thinking.py`), the auth store + Pi-order request-auth + availability gate (`native/auth_store.py`), the stdlib OAuth registry for Anthropic/Copilot/Codex (`native/oauth_providers.py`), `--list-models [search]`, the full-catalog `/model` selector, and direct `/model <ref>` through the shared resolver all ship. Catalog-driven product construction (`native/provider_construction.py` via `current_provider`/`provider_for`) makes a models.json custom provider run a real turn using catalog baseUrl/model/auth/headers/routing/thinking for the `openai-completions` family; `scripts/parity_checks/provider_catalog_conformance.py --json` now covers Verification-Plan items 1-19 incl. item 18 (product construction with fake HTTP). Remaining: catalog-driven construction for the non-completions API families and `pipy run` one-shot construction. Spec in `docs/provider-catalog.md`. |
