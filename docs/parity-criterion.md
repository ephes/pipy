...
| Extension/package platform | 🟡 Pi-shaped core, not Pi-equivalent | Runtime skills/templates/custom commands/themes ship. The Python extension API now has discovery/activation, commands, shortcuts, lifecycle/input/prompt hooks, `tool_call` gates, extension tools, `tool_result` transforms, minimal `ctx.ui.notify`, golden conformance, provider-registration mechanics, and a local-path package CLI. Remaining Pi gaps: package runtime composition, rich UI/rendering/session hooks, dynamic tool/model/thinking controls, `user_bash`/provider-payload hooks, state/session-manager helpers, `--extension` flags, remote packages, update, and supply-chain policy. Source: `docs/extension-api.md`. |
