...
| Extension/package platform | 🟡 Pi-shaped core, not Pi-equivalent | Runtime skills/templates/custom commands/themes ship. The Python extension API now has discovery/activation, commands, shortcuts, lifecycle/input/prompt hooks, `tool_call` gates, extension tools, `tool_result` transforms, minimal `ctx.ui.notify`, simple `ctx.ui` select/input/confirm/editor/status/working primitives, golden conformance, provider registration wired into the native catalog/model selector, a local-path/managed-git package CLI, package runtime composition, package `update`, live-session hooks/controls, dynamic extension flags for tool-loop `pipy repl`, custom session-entry/message rendering, render-once custom tool renderers (`render_call`/`render_result`), persistent chrome widgets, rich message renderers, editor text helpers, theme controls, session-manager read views plus name/label actions, autocomplete provider wrappers, the Pi-shaped custom editor component store, tool-output expansion controls, footer-data snapshots, idle custom-message delivery, and per-run source-loading flags: installed packages contribute extensions/skills/prompts/themes through discovery at lowest precedence with `+/-pattern` filters, and explicit CLI paths can load those resource kinds while default discovery is disabled. Remaining Pi gaps: full custom editor rendering/input integration, live per-frame component re-rendering, reactive footer branch-change delivery, richer multi-widget UI/message components, live tool-render invalidation beyond the render-once snapshot, OAuth-provider extension registration, broader dynamic-flag integration, PyPI/npm package sources, and broader supply-chain policy. Source: `docs/extension-api.md`. |
