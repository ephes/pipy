...
| JSON/RPC automation | 🟡 29-command baseline shipped | Pi-shaped JSON/print/RPC modes ship with queueing, abort, bash, session operations, and introspection. Current Pi has 31 commands; `get_entries`, `get_tree`, and fully-settled lifecycle semantics remain. Spec/gate in `docs/automation-rpc.md`. |
