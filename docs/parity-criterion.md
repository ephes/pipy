...
## Current Stage (refreshed 2026-07-17)
...
Current Pi moved several post-baseline surfaces. Project trust,
...
ship. Durable TUI-only extension entry renderers and cache-friendly dynamic-tool
loading for Anthropic plus OpenAI/Codex Responses now ship too.
Distribution/package polish remains yellow because Pi's bare update is
self-only with `--all`; Kimi deferred tools remain a separate gap. The locked
score intentionally does not change as those larger surfaces move.
...
| A6 | google (Gemini Generative AI) | ✅ | `test -f src/pipy_harness/native/providers/google_generative_ai.py` |
| A7 | google-vertex | ✅ | `test -f src/pipy_harness/native/providers/google_vertex.py` |
...
| E2 | Session compaction (summarize/trim retained context) | ✅ | **behavior check**: `grep -q 'def _compact_if_needed' src/pipy_harness/native/repl/loop_step.py && uv run python scripts/parity_checks/compaction_behavior.py` (drives the tool-loop adapter with `/compact`; proves a `native.session.compacted` event with positive compaction/dropped-group counters and that canonical history compaction never orphans a tool result). Automatic compaction is owned by `repl/loop_step.py`; the manual command routes through `SessionCommandEffects` to the shared compaction effect. Focused product regressions also prove automatic compaction operates on durable history while multiple `deliverAs=nextTurn` messages remain a detached identity overlay on every provider iteration of exactly the next run, absent from its canonical result, additional product `MessageEntry` records, and the metadata-only archive; the original bounded `CustomMessageEntry` remains durable. |
...
| E6 | Settings/config panel | ✅ | `grep -q 'def drive_settings_dialog' src/pipy_harness/native/repl/settings_actions.py && grep -q 'def _settings' src/pipy_harness/native/repl/provider_config_commands.py`. The current command route is `BuiltinCommandInterpreter` → `ProviderConfigurationCommandEffects._settings` → `repl/settings_actions.py::drive_settings_dialog` for a terminal or `tool_loop_settings_overlay_lines` headlessly. |
