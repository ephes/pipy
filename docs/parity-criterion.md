...
| Export/share/distribution/package polish | ✅ baseline shipped | Product HTML export, active-branch JSONL export, import-and-resume, private gist share, top-level `--export`, self-update planning, and install docs ship through `docs/export-distribution.md` and pass `scripts/parity_checks/export_distribution_conformance.py --json`. Remote package-source updates remain a separate extension-platform follow-on. |
