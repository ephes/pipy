...
| Provider/model catalog | 🟡 foundation shipped, product wiring incomplete | A pipy-owned built-in catalog (`native/catalog.py`/`catalog_data.py`, multiple rows per provider), the layered matcher (`native/model_resolver.py`), the `models.json` custom-provider/override loader with routing/compat deep-merge and `refresh`/`register_provider` (`native/models_json.py`), thinking helpers (`native/thinking.py`), the auth store + Pi-order request-auth helpers and availability gate (`native/auth_store.py`), the stdlib OAuth registry for Anthropic/Copilot/Codex (`native/oauth_providers.py`), `--list-models [search]`, and full-catalog `model_options()` ship and pass the current helper-layer `scripts/parity_checks/provider_catalog_conformance.py --json` gate. Closeout review found missing product wiring: provider construction still bypasses `NativeModelSpec`/`models.json`, resolved auth/headers/routing are not applied to provider calls, thinking is not mapped into `ProviderRequest`, and direct `/model <ref>` does not use the shared resolver. Spec + fix-up plan in `docs/provider-catalog.md`. |
