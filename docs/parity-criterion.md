...
| Provider/model catalog | 🟢 catalog shipped, gated | A pipy-owned built-in catalog (`native/catalog.py`/`catalog_data.py`, multiple rows per provider), the layered matcher (`native/model_resolver.py`), the `models.json` custom-provider/override loader with routing/compat deep-merge and `refresh`/`register_provider` (`native/models_json.py`), thinking levels (`native/thinking.py`), the auth store + Pi-order request-auth resolution and availability gate (`native/auth_store.py`), the stdlib OAuth registry for Anthropic/Copilot/Codex (`native/oauth_providers.py`), `--list-models [search]` and the full-catalog `/model` selector (`native/catalog_state.py`), and the ds4 `models.json` reframe (`native/ds4.py`) all ship and pass `scripts/parity_checks/provider_catalog_conformance.py --json` (18/18). Remaining product follow-ons: `/scoped-models` and `--models` Ctrl+P live cycling, plus live Anthropic/Copilot login UX beyond the tested OAuth core. Spec + gate in `docs/provider-catalog.md`. |
