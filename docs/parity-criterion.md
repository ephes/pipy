...
ship. Durable TUI-only extension entry renderers and the Anthropic-native
dynamic-tool-loading slice now ship too. Distribution/package polish remains
yellow because Pi's bare update is self-only with `--all`; OpenAI Responses and
Kimi deferred-tool shapes remain separate gaps. The locked score intentionally
does not change as those larger surfaces move.
...
| Extension/package platform | 🟡 Pi-shaped core, not Pi-equivalent | The Python runtime ships discovery/activation, commands/shortcuts/flags, tools and core hooks, provider registration, local/managed-git packages, rich message/tool/chrome and durable-entry rendering, editor/theme/session helpers, custom editor integration, footer data, live UI invalidation, custom-message delivery, OAuth-provider login, project-trust APIs, `before_provider_headers`, true-idle `agent_settled`, and Anthropic-native cache-friendly dynamic tool loading. Current gaps include OpenAI Responses/Kimi deferred-tool shapes, full component/overlay parity, live tool-render invalidation, richer/RPC UI, and remote package ecosystems. Source: `docs/extension-api.md`. |
