...
| Product TUI/editor workflow | 🟡 daily-driver parity, one known input bug | Inline TUI, slash menu, `/settings`, `/model`, history, paste, undo/redo, resize, `/copy`, `/scoped-models`/Ctrl+P cycling, `@` file picker, path completion, clipboard/drag image references, `!`/`!!`, thinking/model hotkeys, output/thinking folding, queued steering/follow-up, overlays, mouse-selection invariant, and true provider-request cancellation ship. Remaining known Pi-behavior gap: long editable prompts still horizontally scroll in one physical row instead of soft-wrapping inside the input frame. Specified in `docs/tui-workflow.md`. |
| Full session-tree workflow | ✅ shipped | The private full-transcript native session tree (`pipy_harness.native.session_tree` + `session_tree_commands`) ships and passes `scripts/parity_checks/session_tree_conformance.py --json`: full-history `/tree`, `/fork`, `/clone`, `/session`, `/name`, `/new`, `/resume`, durable `/compact`, branch summaries, startup session flags, and the archive-privacy split. Design + gate in `docs/session-tree.md`. HTML export/share are now the selected next parity topic (`docs/export-distribution.md`). |
| Extension/package platform | 🟡 Pi-shaped core, not Pi-equivalent | Runtime skills/templates/custom commands/themes ship. The Python extension API now has discovery/activation, commands, shortcuts, lifecycle/input/prompt hooks, `tool_call` gates, extension tools, `tool_result` transforms, minimal `ctx.ui.notify`, golden conformance, provider-registration mechanics, a local-path package CLI, and package runtime composition: installed local-path packages contribute extensions/skills/prompts/themes through discovery at lowest precedence with `+/-pattern` filters. Remaining Pi gaps: rich UI/rendering/session hooks, dynamic tool/model/thinking controls, `user_bash`/provider-payload hooks, state/session-manager helpers, `--extension` flags, remote packages, update, and supply-chain policy. Source: `docs/extension-api.md`. |
| Provider/model catalog | 🟢 catalog + implemented-family product construction, gated | A pipy-owned built-in catalog (`native/catalog.py`/`catalog_data.py`), the layered matcher (`native/model_resolver.py`), the `models.json` custom-provider/override loader with routing/compat deep-merge and `refresh`/`register_provider` (`native/models_json.py`), thinking (`native/thinking.py`), the auth store + Pi-order request-auth + availability gate (`native/auth_store.py`), the stdlib OAuth registry for Anthropic/Copilot/Codex (`native/oauth_providers.py`), `--list-models [search]`, the full-catalog `/model` selector, and direct `/model <ref>` through the shared resolver all ship. Catalog-driven product construction (`native/provider_construction.py` via `current_provider`/`provider_for`) now covers the implemented catalog-constructed adapter families, one-shot runs, and startup resolution; `scripts/parity_checks/provider_catalog_conformance.py --json` passes items 1-24. Remaining work is adapter/product polish: live Anthropic/Copilot login UX, Vertex API-key auth, Anthropic adaptive thinking, Azure URL/api-version parity, broader local-provider maturity, and extension-registered providers after the extension API exists. Spec in `docs/provider-catalog.md`. |
...
| Export/share/distribution/package polish | 🟡 selected next topic | Pipy has metadata-only `pipy-session export`, local-path package install/list/config, and `/changelog`; Pi has product HTML export, active-branch JSONL export, import-and-resume, private gist share, `--export`, remote package/update flows, and documented installs. Product export/import/share/distribution is the selected next topic in `docs/backlog.md` and specified in `docs/export-distribution.md`; the export conformance gate still needs to be added. |
