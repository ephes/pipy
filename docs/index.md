...
3. User-facing terminal setup: [Terminal Setup](terminal-setup.md) and
   [tmux Setup](tmux.md).
4. [Architecture](architecture.md): the current runtime, diagrams, codebase
   map, and the isolation boundary between domain logic and adapters.
5. [Pi Parity](pi-parity.md): what has already been slopforked from Pi, what
   remains, and how pipy's architecture differs from Pi's.
6. [Parity Plan](parity-plan.md): the clear plan to reach real Pi parity —
   command/flag matrices, accidental-surface cleanup, and big-topic spec index.
7. [Pi-Mono Gap Audit](pi-mono-gap-audit.md): the latest ranked comparison
   against the local Pi checkout, with implementation contracts for the largest
   remaining gaps.
8. [Harness Spec](harness-spec.md): detailed design rationale, event
   vocabulary, native runtime direction, adapter boundaries, and deferred
   design.
9. [Python SDK and Headless Embedding](sdk.md): the current in-process Python
   embedding surface and how it relates to JSON/RPC automation.
10. Big-topic parity specs (target designs, one per large surface):
...
11. [Session Storage](session-storage.md): the metadata-only catalog utility.
...
12. [Backlog](backlog.md): current product planning, completed slices,
