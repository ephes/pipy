...
| Extension / package platform (Python extensions, tools/commands/providers/keybindings/UI hooks, install/update/list/config) | [extension-api.md](extension-api.md) | 🟡 slices 1–9 shipped — discovery/inventory + activation + command dispatch + all hooks (`tool_call`/lifecycle/input/`before_agent_start`/`tool_result`) + tool registration + `ctx.ui.notify`; golden conformance ext, provider registration, package CLI pending | `extension_{discovery,activation,dispatch,tool_call,lifecycle,input_hooks,tools,tool_result,ui_notify}_conformance.py --json`; golden conformance extension (later slices, in spec) |
