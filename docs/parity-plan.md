...
| Extension / package platform (Python extensions, tools/commands/providers/keybindings/UI hooks, install/update/list/config) | [extension-api.md](extension-api.md) | 🟡 slices 1–2 shipped — discovery/inventory (`pipy_harness.native.extensions`) + activation/`register_command` (`extension_runtime`, public `pipy_harness.extensions`); dispatch/hooks/providers/packages pending | `extension_discovery_conformance.py` + `extension_activation_conformance.py --json`; golden conformance extension (later slices, in spec) |
