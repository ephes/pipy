...
| Extension / package platform (Python extensions, tools/commands/providers/keybindings/UI hooks, install/update/list/config) | [extension-api.md](extension-api.md) | 🟡 slices 1–5 shipped — discovery/inventory + activation/`register_command` + live REPL command dispatch + `tool_call` policy hook + lifecycle events (`api.on(...)` observers via `_ExtensionAwareEmitter`); input/before_agent_start hooks, tools, providers, packages pending | `extension_{discovery,activation,dispatch,tool_call,lifecycle}_conformance.py --json`; golden conformance extension (later slices, in spec) |
