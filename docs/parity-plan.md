...
| TUI / editor workflow depth (fuzzy `@` picker, path completion, image paste, `!`/`!!`, scoped-model cycling, thinking hotkeys, folding, queued steering, mouse selection, true cancellation) | [tui-workflow.md](tui-workflow.md) | 🟡 daily-driver basics ship | real-PTY tests + conformance gate (in spec) |
