...
| Extension / package platform (Python extensions, tools/commands/providers/keybindings/UI hooks, install/update/list/config) | [extension-api.md](extension-api.md) | 🟡 slice 1 (discovery + manifest inventory, no execution) shipped — `pipy_harness.native.extensions`; activation/dispatch/hooks/providers/packages pending | `scripts/parity_checks/extension_discovery_conformance.py --json` (slice 1); golden conformance extension (later slices, in spec) |
