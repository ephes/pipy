...
2. **Project trust** (selected next) — first write a design that pins trust-store ancestry,
...
3. **RPC current delta** ([automation-rpc.md](automation-rpc.md)) — **shipped**
   2026-07-14: Pi's read-only `get_entries`/`get_tree` commands and true-idle RPC
   `agent_settled`. Residual independent slices are JSON-mode `agent_settled`,
   true in-turn injection, and RPC extension UI (including the extension-surface
   settled hook).
