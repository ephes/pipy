...
| Extension / package platform (Python extensions, tools/commands/providers/keybindings/UI hooks, install/update/list/config) | [extension-api.md](extension-api.md) | 🟡 slices 1–10 shipped — discovery/inventory + activation + dispatch + all hooks + tool registration + `ctx.ui.notify` + golden conformance ext (`/pipy-extension-conformance`); provider registration + package CLI pending | the `extension_*_conformance.py` gates incl. `extension_conformance_gate.py --json`; golden conformance extension (later slices, in spec) |
