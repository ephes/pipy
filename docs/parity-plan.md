...
| Settings / config / keybindings (global + project `settings.json`, `keybindings.json`, scoped models, system-prompt files, resource toggles, `/reload`, `/changelog`, version/update) | [settings-config.md](settings-config.md) | 🟡 June baseline shipped and the 17-check gate passes. The project-trust design shipped 2026-07-15; trust core/settings-resource gating is the next runtime slice, followed by interactive/package and extension integration. Cache-miss notices, automatic theme mode, and more live display/editor settings remain explicit follow-ons. (`max` thinking shipped 2026-07-14, incl. `defaultThinkingLevel` acceptance.) | `scripts/parity_checks/settings_config_conformance.py --json` |
...
2. **Project trust** — the design shipped 2026-07-15
   ...
   Trust core plus the settings/resource gate is selected next; interactive and
   package-management integration, then extension decision/read APIs, remain
   separate slices so security policy is not mixed with package UI.
