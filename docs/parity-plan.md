...
| Extension / package platform (Python extensions, tools/commands/providers/keybindings/UI hooks, install/update/list/config) | [extension-api.md](extension-api.md) | 🟡 slices 1–3 shipped — discovery/inventory + activation/`register_command` + live REPL command dispatch (`dispatch_extension_command` wired into `tool_loop_session`); tool_call/lifecycle hooks, tools, providers, packages pending | `extension_discovery_conformance.py` + `extension_activation_conformance.py` + `extension_dispatch_conformance.py --json`; golden conformance extension (later slices, in spec) |
