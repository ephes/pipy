...
| Extension / package platform (Python extensions, tools/commands/providers/keybindings/UI hooks, install/update/list/config) | [extension-api.md](extension-api.md) | 🟡 slices 1–7 shipped — discovery/inventory + activation + command dispatch + `tool_call` hook + lifecycle + input/before_agent_start hooks + model-visible tool registration (`api.register_tool`); tool_result hooks, UI, providers, packages pending | `extension_{discovery,activation,dispatch,tool_call,lifecycle,input_hooks,tools}_conformance.py --json`; golden conformance extension (later slices, in spec) |
