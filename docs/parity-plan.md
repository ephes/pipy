...
1. **Extension / package platform follow-ons**
   ...
   Remaining work includes the remaining rich extension UI/rendering pieces (a
   custom editor component — item D, live per-frame component
   ...
   plus the deferred message-entry surface beyond rendering on append
   (replay-on-resume, `send_message`/`deliverAs`/`triggerTurn`, rendering a
   `CustomMessageEntry`); persistent chrome widgets (item B) and rich message
   renderers (item C) now ship), broader dynamic-flag
