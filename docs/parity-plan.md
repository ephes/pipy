...
| Extension / package platform (Python extensions, tools/commands/providers/keybindings/UI hooks, install/update/list/config) | [extension-api.md](extension-api.md) | 🟡 slices 1–4 shipped — discovery/inventory + activation/`register_command` + live REPL command dispatch + `tool_call` policy hook (`api.on`/`ToolBlock` wired into the tool loop); lifecycle/input hooks, tools, providers, packages pending | `extension_{discovery,activation,dispatch,tool_call}_conformance.py --json`; golden conformance extension (later slices, in spec) |
