...
| Extension / package platform (Python extensions, tools/commands/providers/keybindings/UI hooks, install/update/list/config) | [extension-api.md](extension-api.md) | 🟡 slices 1–8 shipped — discovery/inventory + activation + command dispatch + `tool_call`/lifecycle/input/`before_agent_start`/`tool_result` hooks + tool registration; UI notify, golden conformance, providers, packages pending | `extension_{discovery,activation,dispatch,tool_call,lifecycle,input_hooks,tools,tool_result}_conformance.py --json`; golden conformance extension (later slices, in spec) |
