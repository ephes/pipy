...
| Extension / package platform (Python extensions, tools/commands/providers/keybindings/UI hooks, install/update/list/config) | [extension-api.md](extension-api.md) | 🟡 slices 1–6 shipped — discovery/inventory + activation + command dispatch + `tool_call` hook + lifecycle events + input/before_agent_start hooks & `send_user_message`; model-visible tools, tool_result hooks, providers, packages pending | `extension_{discovery,activation,dispatch,tool_call,lifecycle,input_hooks}_conformance.py --json`; golden conformance extension (later slices, in spec) |
