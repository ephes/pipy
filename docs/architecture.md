...
| Tool-loop session | `src/pipy_harness/native/tool_loop_session.py` | Bounded model-driven REPL and current composition root: canonical run/turn/message/tool events, terminal/RPC wait adaptation, queued-input storage and promotion, rendering and extension ordering, diagnostics, provider/model pricing lookup, and concrete production-registry construction. Per-call capabilities, exact request snapshots, active input, usage/history mechanics, runtime effects, request/tool policy callbacks, immutable budget/malformed counters, and zero-retry status normalization now traverse canonical ports and product adapters; each provider completion delegates to the provider-turn executor. The session still owns compaction policy/summary injection, binding semantic extension hooks, actual durable writes, queue storage/priority/reservation/lifecycle, and the provider/tool cycle. Request-only `deliverAs=nextTurn` context never enters canonical history. Full cycle ownership leaves in 2.2b.5c–d; persistence write ownership leaves in Phase 3.3. The native session tree is the full-content record; there is no transcript sidecar. |
