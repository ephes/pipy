...
| Provider-turn executor | `src/pipy_harness/native/agent/loop.py` | UI-free synchronous execution of one provider completion. It publishes text/reasoning deltas as canonical events and owns the optional worker, `CancelToken`, exact cancellation/completion ordering, bounded cleanup, late-delta admission gate, and typed result-or-cancellation outcome. Callers inject terminal or external-abort wait policy; the module imports only provider/cancellation ports, native value objects, and canonical agent contracts and is not eagerly re-exported by `native.agent`. Full provider/tool-cycle ownership remains Phase 2.2b. |
...
| Tool-loop session | `src/pipy_harness/native/tool_loop_session.py` | Bounded model-driven REPL and current composition root: canonical run/turn/message/tool events, terminal/RPC wait adaptation, queued-input storage and promotion, rendering and extension ordering, tool cycles and `--tool-budget` enforcement, malformed-call streak fatal-after-three, zero-retry policy, provider/model pricing lookup, and concrete production-registry construction. Per-call capability access goes through the canonical port and product facade; exact request construction/hook narrowing/returned-call authorization go through the active-input binding, request snapshot, and product adapter; each provider completion delegates to the provider-turn executor; usage and history mechanics delegate to canonical helpers. Provider-cycle durable message requests, usage publication, and eligible queued-input access now traverse canonical runtime ports with late-bound product adapters. The session still owns compaction policy/summary injection, semantic tool hooks, budgets, diagnostics, aggregate counters, actual durable writes, queue storage/priority/reservation/lifecycle, and the provider/tool cycle. Request-only `deliverAs=nextTurn` context no longer enters canonical history or defers automatic compaction. Full cycle ownership leaves in 2.2b.5; persistence write ownership leaves in Phase 3.3. The native session tree is the full-content record; there is no transcript sidecar. |
