...
| Tool-loop session | `src/pipy_harness/native/tool_loop_session.py` | Bounded model-driven REPL and current composition root: canonical run/turn/message/tool events, terminal/RPC wait adaptation, queued-input storage and promotion, provider-request construction, rendering and extension ordering, tool cycles and `--tool-budget` enforcement, malformed-call streak fatal-after-three, zero-retry/current usage policy, and the production tool registry. Per-call tool execution delegates to the tool executor and each provider completion delegates to the provider-turn executor; the full provider/tool cycle remains here until Phase 2.2b. The native session tree is the full-content record; there is no transcript sidecar. |
