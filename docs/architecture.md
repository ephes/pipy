...
| Provider-turn executor | `src/pipy_harness/native/agent/provider_turn.py` | UI-free synchronous execution of one provider completion. It publishes text/reasoning deltas as canonical events and owns the optional worker, `CancelToken`, exact cancellation/completion ordering, bounded cleanup, late-delta admission gate, and typed result-or-cancellation outcome. Callers inject terminal or external-abort wait policy; the module imports only provider/cancellation ports, native value objects, and canonical agent contracts and is not eagerly re-exported by `native.agent`. The `native.agent.loop` name is reserved for the full provider/tool-cycle owner introduced by Phase 2.2b.5c. |
