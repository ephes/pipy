...
- `--archive-transcript` is an opt-in flag that writes raw loop turns to a
  sensitive sidecar at `~/.local/state/pipy/transcripts/<id>.jsonl` outside
  the metadata-first session archive. The sidecar is excluded from
...
- Ordinary non-command input in `--repl-mode no-tool` makes bounded no-tool
  provider turns with small in-memory conversation context.
...
- In `--repl-mode tool-loop`, the bounded model-driven loop dispatches
  ...
  `grep`, `find`, `write`, `edit`, `edit_diff`, and `truncate` tools defined under
...
| Model-driven tools | `src/pipy_harness/native/tools/read.py`, `ls.py`, `grep.py`, `find.py`, `write.py`, `edit.py`, `edit_diff.py`, `truncate.py`; disabled shell helper `bash.py` | Production registry tools for the bounded tool-loop. Filesystem tools reuse `_validate_workspace_relative_path`, `_is_ignored_or_generated`, `_is_relative_to`, and `_resolved_relative_label` from `read_only_tool.py` for `.git`/symlink default-deny, and stat-gate oversized file reads before loading content. `bash.py` is not registered for model use until a real shell sandbox preserves secret isolation and `.git` default-deny; `truncate` is pure transformation only. |
