...
| Tool-loop session | `src/pipy_harness/native/tool_loop_session.py` | Bounded model-driven REPL and current composition root: canonical run/turn/message/tool events, terminal interruption adaptation, rendering and extension ordering, `--tool-budget` enforcement, malformed-call streak fatal-after-three, and the production tool registry. Per-call execution delegates to the reusable tool executor. The native session tree is the full-content record; there is no transcript sidecar. |
