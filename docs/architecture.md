...
| Tool-loop session | `src/pipy_harness/native/tool_loop_session.py` | Bounded model-driven REPL and current composition root: canonical run/turn/message/tool events, terminal/RPC wait adaptation, queued-input storage and promotion, provider-request construction, rendering and extension ordering, tool cycles and `--tool-budget` enforcement, malformed-call streak fatal-after-three, zero-retry policy, provider/model pricing lookup, and concrete production-registry construction. Per-call capability access goes through the canonical port and product facade, each provider completion delegates to the provider-turn executor, provider-neutral usage delegates to the canonical accumulator, and mechanical message-history reduction delegates to canonical agent history. The session still owns compaction settings/triggers/summary injection, extension hook dispatch and ordering, budgets, diagnostics, aggregate counters, durable session-tree writes, and the final provider-request snapshot. It defers automatic compaction while transient extension `deliverAs=nextTurn` context is attached so a history-index shift cannot leak that one-turn context into a later provider request. The remaining request/effect/input and full provider/tool-cycle seams leave here in Slices 2.2b.4–2.2b.5. The native session tree is the full-content record; there is no transcript sidecar. |
