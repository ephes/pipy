...
classification, error-normalization consolidation, streaming transport, and the
per-provider `UrllibJsonHTTPClient`/exception-hierarchy collapse remain owned by
each adapter and land with the protocol-family migrations in Slice 5.2.
