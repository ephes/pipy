...
consolidated (that stays the later `_chat_completions_shared` backlog cut). ds4
...
`_chat_completions_shared` backlog cut, which now owns Mistral and OpenRouter
...
`_chat_completions_shared` backlog cut). The superseded top-level path is
