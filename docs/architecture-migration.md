...
retention rather than a hidden write-only alias. `native_provider_available` stays as
`catalog_state`'s single availability owner, and `NATIVE_PROVIDER_REGISTRY` /
