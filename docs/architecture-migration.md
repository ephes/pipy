...
> is also completed/reconciled historical evidence.
>
...
> both theme sources are `pi`, and pre-commit is absent. Slice 14 stress evidence remains focused
> 20x, groups 10x, PTY smoke 5x, then the full check; the latest PTY smoke is
> 8/8.
>
> The completed/reconciled disposition is unchanged. The next boundary remains
...
| 7.5g (relocate external-abort provider-turn adapters) | This commit — `refactor: relocate external abort provider adapters` | Seventh composition-root slimming cut. The runtime-checkable `_AbortCallbackSignal`, `_StartGatedProvider`, and `_wait_for_external_abort` move mechanically from `native.tool_loop_session` into the existing strict, UI-free `native.agent.provider_turn` owner beside `ProviderTurnWaiter` and `ProviderTurnExecutor`; the root imports those private seams and retains only terminal-UI interruption translation. The superseded definitions are deleted with no alias or public `native.agent` re-export, and the direct post-done accepted-abort test moves to the owner. Callback registration still precedes provider start; the start event is still released from the same `finally`; immediate and post-done abort checks, accepted-abort state, cancel setting, 0.05-second polling, callback unregister, ProviderPort properties, blocking start wait, request/sink/token forwarding, and RPC result/exception ordering are unchanged. The composition root shrinks from 6,100 to 6,016 lines. Repository C901 remains 119/59 and `src` `type: ignore` remains 28; no behavior/export, dependency, unchecked `Any`, ignore, pin, or Mypy exclusion changes. Focused provider-turn/session/automation verification passed 150 tests and architecture-boundary verification passed 169; final `just check` passed Ruff, Mypy, and 4,514 tests (2 skipped); `just docs-build` passed. Review: Pi GPT-5.6 Sol, 1 round, 0 findings, explicit CLEAN. |
...
| 7.7f (decompose workspace and pure-UI leaf owners) | This commit — `refactor: decompose workspace and ui leaves` | Seventh directional C901 burn-down batch. `native.workspace_context._load_first_candidate` now delegates directory resolution, candidate file/resolution/containment/dedup validation, and successful read/materialization; candidate precedence, every OSError fallthrough, symlink escape defense, seen-path mutation only after a successful read, UTF-8 replacement, truncation/hash metadata, and label-call timing remain unchanged. The ancestor label callback uses a typed `functools.partial` over the existing function after the full Mypy gate exposed the newly annotated lambda. `native.ui.state.reduce` separates assistant lifecycle/start/completion from stateless tool-event reduction while preserving state identity/transitions and buffered-before-complete order. `native.ui.rendering.RenderingAgentEventAdapter` separates assistant and tool decision application while keeping state assignment, renderer/sink order, and exhaustive `assert_never`. A new workspace characterization pins read-error fallthrough, seen-path mutation, and label timing. All three owners are clean and their pins are deleted, lowering repository C901 119/59 -> 116/56 (`src` 103/50 -> 100/47). No workspace/privacy/security policy, UI contract/export, terminal behavior, dependency, unchecked `Any`, `type: ignore`, Mypy exclusion, or pin growth. Focused verification passed 89 tests (2 skipped); final `just check` passed Ruff, Mypy, and 4,515 tests (2 skipped); `just docs-build` passed. Review: Pi GPT-5.6 Sol, 1 round, 0 findings, explicit CLEAN. |
| 7.7g (decompose core read/write/list tool owners) | This commit — `refactor: decompose core file tools` | Eighth directional C901 burn-down batch. `ReadTool` separates typed path resolution and ignore policy, existence/file preflight, max-size-before-read I/O, NUL/UTF-8/control/secret validation, and line-before-byte excerpting; exact error class/message order and UTF-8 boundary behavior remain unchanged. `WriteTool` separates path validation before content typing, workspace/label/ignore/existing/parent preflight, UTF-8 create-only mutation, and post-success unified-diff streaming; no stderr call occurs before a successful write. `LsTool` separates `"."`/reference-root target resolution, existence/type/sorted listing, ignored/resolved-contained child filtering, type classification, visible-row capping, and marker/empty output formatting. Typed private target/failure values preserve ToolArgumentError versus provider-visible error-result boundaries and correlation IDs. New tests pin line/byte UTF-8 truncation, write validation order, and ignored-child filtering before the row cap. All three owners are clean and their pins are deleted, lowering repository C901 116/56 -> 113/53 (`src` 100/47 -> 97/44). No tool schema/path/security/archive policy, public export, provider-visible text, filesystem semantics, dependency, unchecked `Any`, `type: ignore`, Mypy exclusion, or pin growth. Focused verification passed 61 tests; final `just check` passed Ruff, Mypy, and 4,518 tests (2 skipped); `just docs-build` passed. Review: Pi GPT-5.6 Sol, 1 round, 0 findings, explicit CLEAN. |
| 7.5h (relocate the reload-unavailable provider adapter) | This commit — `refactor: relocate reload failure provider` | Eighth composition-root slimming cut. `UnavailableAfterReloadProvider` moves from `native.tool_loop_session` into the already-strict `native.repl_state` owner beside `_FailedExtensionProvider` and the provider/model selection state. The root imports and constructs the adapter from the same active selection when reload removes its extension provider; the original class and now-unused root imports are deleted with no alias or duplicate. The adapter retains `supports_tool_calls=True`, ignores stream/reasoning sinks and cancellation, uses the shared UTC clock helper, and returns the same failed provider result with the exact `ProviderUnavailableAfterReload` type and forwarded message. A direct owner test pins identity, capability, sink/cancellation indifference, failure metadata, and current UTC timing; existing reload integration tests remain unchanged. The composition root shrinks 6,016 -> 5,989 lines. Repository C901 remains 113/53 (`src` 97/44) and `src` `type: ignore` remains 28; no behavior/export, dependency, unchecked `Any`, ignore, pin, or Mypy exclusion changes. Focused verification passed 137 tests; final `just check` passed Ruff, Mypy, and 4,519 tests (2 skipped); `just docs-build` passed. Review: Pi GPT-5.6 Sol, 1 round, 0 findings, explicit CLEAN. |
| 7.7h (decompose typed state/package/provider leaf owners) | This commit — `refactor: decompose typed state and package leaves` | Ninth directional C901 burn-down batch. `native.coding.state.require_exact_agent_message` keeps the exact-type dispatcher and delegates to message-family validators for user content, assistant content/tool calls, and tool-result ownership/correlation/additional-tool fields; exception types, field paths, recursive immutability, size bounds, and first-failure order are unchanged. `native.package_resources.resolve_package_roots` uses one typed ordered accumulator and separates normalized-entry handling, local/git/unsupported-remote resolution, safe disabled-source recording, successful-source deduplication, and manifest/resource materialization; the subtle timing remains unchanged: unresolved sources are reported on every entry, while a resolved source is marked seen before manifest loading and an invalid manifest is reported once. `native.provider_registry.native_provider_available` separates static/login, named-environment, and compound cloud policies while retaining auto-default filtering, environment truthiness, Vertex API-key-or-token/project, Azure endpoint/resource-plus-key, and unknown-policy false. New tests pin validation-family first failures and missing-versus-invalid-source dedup timing. All three strict owners are clean and their pins are deleted, lowering repository C901 113/53 -> 110/50 (`src` 97/44 -> 94/41). No public API/state/package/provider behavior, dependency, unchecked `Any`, `type: ignore`, Mypy exclusion, or pin growth. Focused verification passed 150 tests; final `just check` passed Ruff, Mypy, and 4,521 tests (2 skipped); `just docs-build` passed. The first full run hit the unrelated documented PTY worker-timing flake, which passed immediately in isolation; the full rerun was green. Review: Pi GPT-5.6 Sol, 1 round, 0 findings, explicit CLEAN. |
| 7.5i (relocate the extension-aware agent lifecycle adapter) | This commit — `refactor: relocate extension lifecycle event adapter` | Ninth composition-root slimming cut. Private `_ExtensionLifecycleAgentEventAdapter` moves into `native.extension_hooks`, which already owns lifecycle-hook dispatch. The product composition root still constructs the fixed immediate chain in its exact renderer -> optional automation -> product persistence -> metadata-only workflow archive -> optional caller order, then supplies the completed `SynchronousAgentEventComposite` as one `AgentEventSink`. The relocated adapter synchronously delivers there first, maps only `AgentRunStarted`/`AgentRunCompleted`/`TurnStarted`/`TurnCompleted` to extension lifecycle names afterward, retains mutable hook and copied-flag replacement, carries cwd/UI/notify/driver/trust context, and keeps `agent_settled` extension-only so JSON/RPC protocol synthesis is not duplicated. Failed immediate delivery still prevents later extension callbacks. The root definition is deleted with no alias; direct tests repoint to the owner and add ordering/failure/context coverage. The composition root shrinks 5,989 -> 5,904 lines. Repository C901 remains 110/50 (`src` 94/41) and `src` `type: ignore` remains 28. No canonical event, renderer, automation, persistence/archive privacy, lifecycle, extension export, dependency, unchecked `Any`, ignore, pin, or Mypy exclusion changes. Focused verification passed 305 tests; the full Mypy gate caught one test-only opaque driver annotation, fixed with an explicit `ExtensionUiDriver` cast. Final `just check` passed Ruff, Mypy, and 4,523 tests (2 skipped); `just docs-build` passed. Review: Pi GPT-5.6 Sol, 1 round, 0 findings, explicit CLEAN. |
| 7.7i (decompose agent request/executor and resource dispatch) | This commit — `refactor: decompose agent and resource dispatch` | Tenth directional C901 burn-down batch. `native.agent.request._validate_schema_semantics` separates scalar keywords, exact-string tuple keywords, and recursive properties/items traversal while preserving the original cross-family and depth-first error order and exact messages. `native.agent.tools.ToolExecutor._execute_interruptibly` delegates only the interrupt waiter call, KeyboardInterrupt mapping, invalid-return check, and waiter-failure cancellation/output-gate/bounded-join cleanup; worker creation/start, completion recording before done, cancellation-versus-completion ordering, output-gate timing, joins, error re-raise, and final outcomes remain byte-equivalent in the caller. `native.resources.dispatch_resource_command` retains slash recognition and reserved/unknown fallthrough, delegates `/skill` listing/multi-word lookup/empty rejection/load, and then delegates direct template-before-custom-command resolution with unchanged expansion, metadata, labels, messages, and provider text. Three new parametrized schema cases pin scalar-before-sequence-before-children and first-property-before-items errors; the existing adversarial executor race suite stays green. All three owners are clean and their pins are deleted, lowering repository C901 110/50 -> 107/47 (`src` 94/41 -> 91/38). No agent/tool/resource API, synchronization, command precedence, dependency, unchecked `Any`, `type: ignore`, Mypy exclusion, or pin growth. Focused verification passed 61 tests; final `just check` passed Ruff, Mypy, and 4,526 tests (2 skipped); `just docs-build` passed. Review: Pi GPT-5.6 Sol, 1 round, 0 findings, explicit CLEAN. |
| 7.5j (relocate footer/status effects to terminal chrome) | This commit — `refactor: relocate footer effects to chrome` | Tenth composition-root slimming cut. Private `_ChromeFooterEffects` moves into `native.chrome`, which already owns startup/footer rendering and bottom-status formatting. Precise private callable Protocols describe the session’s bound footer-text and print methods; structural footer-UI and REPL-runtime Protocols allow live footer mutation and slash-menu detection without chrome importing `tool_loop_session` or `tui`. The root constructs the adapter with its current bound `_footer_text` and `_print_footer` methods, preserving those formatting/printing implementations and their monkeypatch seams. Every method still reads live provider/model/turn/tool/usage state at call time; TUI refresh occurs only with a terminal UI, the plain legacy footer remains suppressed for slash-menu runtime, no-usage and with-usage calls retain exact keyword sets, and startup/command/provider-mutation/loop-step timing is unchanged. The root class is deleted with no alias, and the constant-time state characterization repoints to the owner. The composition root shrinks 5,904 -> 5,829 lines. Repository C901 remains 107/47 (`src` 91/38) and `src` `type: ignore` remains 28. No footer/status/TUI/provider/command behavior, public export, dependency, unchecked `Any`, ignore, pin, or Mypy exclusion changes. Focused verification passed 418 tests; final `just check` passed Ruff, Mypy, and 4,526 tests (2 skipped); `just docs-build` passed. Review: Pi GPT-5.6 Sol, 1 round, 0 findings, explicit CLEAN. |
| 7.7j (decompose request auth, fake provider, and Vertex preflight) | This commit — `refactor: decompose auth and vertex preflight` | Eleventh directional C901 burn-down batch. `native.auth_store.resolve_request_auth` delegates credential priority (runtime -> stored API key/OAuth -> environment -> models.json), ordered models.json then model-header resolution/overwrite with the selected header environment, and authHeader missing-key failure/Authorization overwrite. The live fake finding was `FakeNativeProvider.complete` (not `AutomationFakeProvider.complete`): it separates cancellation-before-timestamp entry and scripted wait, successful reasoning-before-text stream emission, final text selection, ordered copied fixture metadata, and programmable tool-call selection before call-counter increment. `GoogleVertexProvider.complete` delegates model then Express/ADC preflight to frozen typed configuration/failure values, preserving Express key rules and ADC project -> access token -> location first failures, quoted endpoint values, auth header/mode/location metadata, and the unchanged body/thinking/HTTP/parse/result path. New tests pin resolver/header order, fake stream/metadata/counter behavior, and exact Vertex failure order. All three owners are clean and their pins are deleted, lowering repository C901 107/47 -> 104/44 (`src` 91/38 -> 88/35). No auth/provider policy, wire/fixture surface, output/tool/usage shape, dependency, unchecked `Any`, `type: ignore`, Mypy exclusion, or pin growth. Focused verification passed 139 tests; final `just check` passed Ruff, Mypy, and 4,531 tests (2 skipped); `just docs-build` passed. The first full run hit the unrelated documented PTY worker-timing flake, which passed immediately in isolation; the full rerun was green. Review: Pi GPT-5.6 Sol, 2 rounds, 1 finding, final explicit CLEAN. Round one found that a reasoning callback could mutate fake status before the separately guarded text stream; the second success check and a regression test restore the original guard timing. |
| 7.7k (decompose native request invariants) | This commit — `refactor: decompose native request invariants` | Twelfth directional C901 slice. `NativeReadOnlyToolRequest`, `NativePatchProposal`, and `NativePatchApplyRequest` delegate pipy-owned identity, policy/sandbox, bounded integer/label, patch operation shape/constraint/path, privacy-storage, and optional-scope invariant families while preserving exact validation order/messages and dataclass shapes. Patch operations still validate fields before rename/delete constraints, record source then target overlap, enforce the distinct-path limit after all operations, validate privacy flags next, and scope last. New tests pin rename self-overlap and per-operation failure before the path-count limit. All three findings are gone and the owner pin is deleted, lowering repository C901 104/44 -> 101/43 (`src` 88/35 -> 85/34). No model/schema/archive/session format, privacy policy, dependency, unchecked `Any`, `type: ignore`, Mypy exclusion, or pin growth. Focused verification passed 56 tests; final `just check` passed Ruff, Mypy, and 4,533 tests (2 skipped); `just docs-build` passed. Review: Pi GPT-5.6 Sol, 1 round, 0 findings, explicit CLEAN. |
| 7.6e (extend strict Mypy to the tool-loop composition root) | This commit — `build: enable strict typing for tool loop session` | Fifth strict-root follow-on. `pipy_harness.native.tool_loop_session` joins the enumerated per-module override. Full-graph Mypy identified seven imported names intentionally exposed as module attributes for compatibility/monkeypatch tests; same-name aliases make those re-exports explicit under `no_implicit_reexport`. No runtime statement, signature, control flow, or type suppression changes. Focused verification passed 288 tests; final `just check` passed Ruff, Mypy, and 4,533 tests (2 skipped); `just docs-build` passed. C901 remains 101/43, `src` `type: ignore` remains 28, and the root remains 5,829 lines. Review: Pi GPT-5.6 Sol, 1 round, 0 findings, explicit CLEAN. |
| 7.6f (extend strict Mypy to the native TUI) | This commit — `build: enable strict typing for native tui` | Sixth and final requested strict-root follow-on. `pipy_harness.native.tui` joins the enumerated per-module override, closing its 28 measured errors through typed tuple construction, chrome-style returns, pending custom-render state, and the custom-render dispatcher. The render-details handoff now exposes a write-only protocol and chooses separate typed terminal/captured sinks; manually injected non-mapping details still flow unchanged to extension renderers and are pinned by a regression test. The first implementation's runtime narrowing was rejected before review; the recovery typed the handoff end-to-end without narrowing. The full architecture gate then caught `NativeToolReplSession.run()` at 805 AST lines, so its sink setup moved to a module-level frozen bundle/helper, restoring the guard to 798 lines without changing behavior. No `strict = true`, sub-flag relaxation, ignore, cast hiding `Any`, exclusion, dependency, C901 pin, renderer/input/TUI behavior, or public surface changes. Focused verification passed 299 tests; final `just check` passed Ruff, Mypy, and 4,534 tests (2 skipped); `just docs-build` passed. The first full run encountered two PTY timing failures; the editor case passed in a paired isolation run, the known multi-tool race passed alone, and the complete rerun was green. C901 remains 101/43, `src` `type: ignore` remains 28, and the root remains 5,829 physical lines. Review: Pi GPT-5.6 Sol, 3 rounds, 2 findings, final explicit CLEAN. Round one caught the mapping-only internal reader type; round two rejected widening the public `ToolRenderContext.details` contract. The final shape keeps extension writes and the public context mapping-typed while isolating legacy opaque reader values at one internal compatibility bridge. |
| 7.5k (relocate the extension activation bundle and projection builder) | This commit — `refactor: relocate extension activation projection` | Eleventh composition-root slimming cut. The pure `_ExtensionRuntime` contribution bundle moves to `native.extension_runtime`; the session-run `_activate_workspace_extensions` discovery/activation/projection builder moves to the existing one-way `native.extension_hooks` aggregation owner. The coordinator rejected the first implementation before review because placing the builder in `extension_runtime` introduced reverse lazy imports into `extension_hooks` and `extension_provider_catalog`; the recovered placement preserves the documented cycle-free direction (`extension_hooks` depends on the runtime, never the reverse). The root imports the two private seams from their owners, the direct source-loading test repoints, and the superseded definitions plus unused imports are deleted with no alias. Discovery inputs, CLI/workspace/default enablement, reserved command/tool names, resource-pattern filtering, activation-batch finalization, activation ordering/fail-closed behavior, outbox list identity, activated-only custom-message filtering, and every projected command/hook/tool/flag/provider/shortcut/renderer contribution are unchanged. The composition root shrinks 5,829 -> 5,659 physical lines. No public re-export, behavior, dependency, unchecked `Any`, ignore, Mypy exclusion, or C901 pin changes. Focused verification passed 352 tests plus fresh-process import-order smoke checks; final `just check` passed Ruff, Mypy, and 4,534 tests (2 skipped). C901 remains 101/43 and `src` `type: ignore` remains 28. Review: Pi GPT-5.6 Sol, 1 round, 0 findings, explicit CLEAN. |
| 7.7l (decompose strict agent, automation, and clipboard leaves) | This commit — `refactor: decompose strict agent automation leaves` | Thirteenth directional C901 batch. `agent.loop._validate_provider_result` delegates the same ordered exact result/status/request-identity, scalar/timestamp/optional-text, optional usage/metadata, and tool-call tuple/item validation families without changing first-failure order or messages. `AutomationAgentEventAdapter._project` delegates run/turn/message, assistant-text, tool-execution, retry, and ignored-bookkeeping event families while preserving cumulative assistant text reset/update timing, exact Pi dictionaries/key insertion order, synchronous one-event emission, and the unsupported-event `TypeError`. `clipboard._default_run_capture` separates bounded deadline/select/read accumulation, process cleanup, and result classification while preserving DEVNULL stdin/stderr, monotonic deadline, +1 overflow cap, OSError/timeout/non-zero-exit results, kill/close/wait timing, and returned bytes. Existing characterization coverage required no change. All three strict owners are clean and their pins are deleted, lowering repository C901 101/43 -> 98/40 (`src` 85/34 -> 82/31). No agent result contract, automation wire/order, clipboard security/time/memory policy, public API, dependency, unchecked `Any`, ignore, Mypy exclusion, or new pin changes. Focused verification passed 95 tests; final `just check` passed Ruff, Mypy, and 4,534 tests (2 skipped). `src` `type: ignore` remains 28 and the root remains 5,659 physical lines. Review: Pi GPT-5.6 Sol, 1 round, 0 findings, explicit CLEAN. |
| 7.5l (relocate the custom-entry renderer adapter to TUI) | This commit — `refactor: relocate custom entry renderer adapter` | Twelfth composition-root slimming cut. Private `_CustomEntryRenderer` moves mechanically into strict `native.tui`, which already owns `ToolLoopTerminalUi`, live custom-component rendering, chrome/theme projection, and the extension message/entry render helpers. Narrow private structural protocols expose only the mutable run-state fields and diagnostic-host method the adapter consumes; `_RunControlState` and `NativeToolReplSession` satisfy them without `tui` importing the root, so `/reload`/session rebinds remain visible through the same live objects. The root imports and constructs the adapter with the same collaborators, and the superseded class plus obsolete imports are deleted with no alias or public export. A new architecture test forbids `native.tui` from importing `native.tool_loop_session`; fresh-process imports pass in both orders. Custom entry/message safe-data projection, session-tree append timing/IDs, live versus captured rendering, styled/plain delivery, display filtering, renderer metadata, branch replay/redraw order, diagnostics, outbox drain/list identity, `deliverAs` handling, `triggerTurn` gating, and coding-input queue timing are unchanged. The root shrinks 5,659 -> 5,382 physical lines. No behavior, dependency, unchecked `Any`, ignore, Mypy exclusion, or C901 pin changes. Focused verification passed 283 tests; final `just check` passed Ruff, Mypy, and 4,535 tests (2 skipped). C901 remains 98/40 and `src` `type: ignore` remains 28. Review: Pi GPT-5.6 Sol, 1 round, 0 findings, explicit CLEAN. |
