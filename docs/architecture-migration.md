...
`message`/`__type` shape rather than the shared nested-`error` shape. Codex
(its own SSE/OAuth transport) remains untouched and lands with the
protocol-family migrations in Slice 5.2.
