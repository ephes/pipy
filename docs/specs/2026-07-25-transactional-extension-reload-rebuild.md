...
Status: R0-reconciled completion contract; R1–R4c are shipped and R3/R4 are
complete; R5a is active/next. The original
...
chrome handles outside the session mutex. Production still lacks terminal
coding-effect/sidecar invocation, generation-bound stale mutation refusal, and
atomic `set_model` admission; R5a–R6 retain those owners.
The
...
and R4c menu/lifecycle/chrome snapshot consumers are shipped; R5a is active/next.
...
| Generation chrome sidecar: retained chrome/listener/autocomplete/editor-component values and closed state, guarded by that sidecar's sink-local guard | R4c immutable bound-operation and reconciliation snapshots after session unlock | retained class-B setters/registrations; R2 candidate close; R4c retired close; R5a terminal close | R2 sink; R3a detached exact handle; R3c3 installation; R4c binding/retired close landed; R5a retains terminal invocation; paint/disposal after unlock |
...
These bounds support the exact 33-slice plan. The shipped prefix now ends at R4c, and the mandatory remaining order begins
with R5a. “Named mechanisms” may list
...
| R4c — **shipped** | Menu/description/shortcut, lifecycle and user-bash inputs, generation-bound retained chrome reads/writes, retired close, and reload composition. Terminal invocation remains R5a. | One generation snapshot per operation. Publication captures the displaced handle under the session mutex; after unlock its sink-local guard performs closed-check/snapshot/detach, and callbacks/disposal/paint/last-reference release run after all guards. The separately stored flag map, temporary lifecycle/flag reload effect, mutable emitter copies, direct runtime contribution reads, and final equivalence arms are gone. |
...
| Closed rejected/retired sidecar drops appends/chrome writes; retired queue handles remain observably undeliverable as today; worker outliving `run()` mutates no class-A or coding-session state | **Queue closure landed in R4a and retired chrome closure in R4c; terminal invocation and class-A/coding-session behavior remain required in R5a/R5b/R6.** |
