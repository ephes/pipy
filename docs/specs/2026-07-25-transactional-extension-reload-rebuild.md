...
Status: R0-reconciled completion contract; R1–R4b are shipped and R3 is
complete; R4c is active/next. The original
...
the R3a/R3b values and invokes the R3c1a–R3c1c owner APIs. R4b now binds tool
advertisement/execution, tool and custom renderer selection, and provider
turn/contribution consumers to one published generation snapshot. Production
still lacks R4c menu/lifecycle/chrome snapshot adoption, generation-bound stale
mutation refusal, and atomic `set_model` admission; R4c–R6 retain those owners.
The
...
plus live queue synchronization, and R4b tool/renderer/provider snapshot
consumers are shipped; R4c is active/next.
...
| `SessionExtensionGeneration` contains runtime plus flags while capability, renderer, lifecycle, provider, menu, and chrome projections publish separately. | **R3a–R3c3 construction/publication, R4a command/request/session-gate, and R4b tool/renderer/provider consumer adoption landed; R4c retains the remaining menu/lifecycle/chrome consumers.** `build_extension_projection()` constructs the detached immutable runtime/flag, command/menu/shortcut, hook, tool/capability, renderer, provider, queue-handle, and exact-chrome families. R4b deletes each converted legacy source and only its tool/renderer/provider equivalence arm after the last consumer moves; every arm for an unconverted source remains. |
| Production operations read `current` per access rather than one snapshot. | **R4a lands command, shortcut, input, before-agent, before-provider, tool-result, session-gate, and renderer-drain adoption; R4b lands tool advertisement/execution, tool/custom renderer, provider-turn, and provider contribution/refresh snapshots; R4c retains menu/lifecycle/chrome.** Barrier tests prove a provider iteration spanning reload keeps its advertised tool, renderer, and already-built provider generation and that the next iteration sees the successor. Direct custom tree/render/input delivery remains R1-compatible and does not consult routing; only coherent drain consultation affects queue side effects. |
...
| C04 | A generation has one immutable contribution/configuration projection. | **Landed for installed startup/reload generations in R3c3, R4a command/request/session-gate consumers, and R4b tool/renderer/provider consumers; R4c retains remaining adoption.** Both production paths call the same R3a builder, and `SessionExtensionGeneration` carries that exact frozen `ExtensionProjection` through the single `SessionGenerationRef` pointer. R4b removes the temporary renderer publication and direct legacy tool/renderer/provider sources only after their final consumers move; lifecycle/menu/chrome legacy sources and arms remain for R4c. |
| C05 | Operations take one generation snapshot at start and keep it for their duration. | **Landed through R4a for command/request/session-gate/outbox-drain and R4b for tool/renderer/provider operations; R4c retains menu/lifecycle/chrome.** `_SessionExtensionOperations` preserves the R4a flag and identity contracts. `_SessionExecutionProjections` takes one mutex snapshot per provider iteration and retains the projected registry/executor, tool-call hooks/flags, filtered renderer map, and current already-built coding provider. Tool workers and renderer pending state retain that value across reload; the next iteration snapshots the successor. Custom message/entry render, replay, redraw, append, and direct-send rendering select one projected map per operation. Startup and refusal provider contribution/refresh paths use one projected tuple. No provider construction, execution, rendering, or callback occurs under the session mutex. The R4a drain method remains unchanged. |
...
| Installed immutable `ExtensionProjection` in `SessionExtensionGeneration` | R4a command/request/session-gate and renderer-drain snapshots, R4b tool/renderer/provider snapshots, plus R4c pending consumers | R3a pure builder; R3c3 startup/reload installation | R3a construction, R3c3 installation, and R4a/R4b bounded consumer migration landed; R4c retains menu/lifecycle/chrome |
...
These bounds support the exact 33-slice plan. The shipped prefix now ends at R4b, and the mandatory remaining order begins
with R4c. “Named mechanisms” may list
...
| --- | --- |
| Retained frozen projection; successful coherent commands/hooks/tools/providers/renderers/flags/shortcuts/UI; derived projection failure | **R3a detached construction and R3c3 installation, R4a command/request/session-gate, and R4b tool/renderer/provider adoption landed; R4c remains**, closed in R7 integration. |
...
| Candidate flag failure keeps runtime/flags | **Landed**, including R2 preservation of old retained chrome/listeners, R3c3 installation, and R4a/R4b bounded snapshot consumption; R4c retains other consumers. |
