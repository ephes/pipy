...
Status: R0-reconciled completion contract; R1–R2 are shipped and R3 is next. The
...
`set_model` admission. Rejected activation now preserves retained chrome because R2 stages candidate
chrome/listener requests and reconciles them only after semantic acceptance.
Complete generation projection and retired-handle binding remain R3/R4c-owned.
...
| Candidate flag failure keeps runtime/flags | **Landed**, including R2 preservation of old retained chrome/listeners; R3/R4c add every projection. |
