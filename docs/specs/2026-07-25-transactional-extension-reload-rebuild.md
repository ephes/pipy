...
Status: R0-reconciled completion contract; R1–R2 and construction-only R3a–R3b
are shipped, R3 remains incomplete, and R3c1 is next. The original Slice 3 text remains historical design
...
R3a nor R3b. The implementation still lacks projection/effect publication and
consumer adoption, production operation snapshots, generation-bound stale
mutation refusal, and atomic `set_model` admission. R3c1–R3c3 and R4a–R4c retain
those owners. The former one-shot R3c contract was non-executable: its source
manifest excluded the real `_ActivationApi` owner while requiring send-path gate
consultation, and the provider catalog, coding session, and provider-selection/
pending-default owners lacked the detached prepare/non-fallible publish ports
required by `PreparedReloadEffects`. `NativeReplProviderState` in
`repl_state.py` is the real selection/default owner; `_ProviderMutationEffects`
currently orchestrates reload selection, fallback, and defaults.
`NativeToolCapabilities` already ships the required `ToolCapabilityState`
prepare/publish APIs and need not be edited. R3c is replaced by ordered R3c1
owner APIs, R3c2 routing seam, and R3c3 composition. This docs correction
...
program to 27 slices, the R3a/R3b/R3c split brought it to 29, and the later
R3c1/R3c2/R3c3 split brings it to exactly 31.
...
| `SessionExtensionGeneration` contains runtime plus flags while capability, renderer, lifecycle, provider, menu, and chrome projections publish separately. | **R3a construction and R3b detached-effect preparation landed; R3c1 owner ports, R3c2 routing, and R3c3 installation and R4a–R4c consumer adoption remain.** `build_extension_projection()` constructs the detached immutable runtime/flag, command/menu/shortcut, hook, tool/capability, renderer, provider, queue-handle, and exact-chrome families. The live generation shape and every legacy consumer remain unchanged. Each equivalence arm stays until the corresponding R4 slice deletes its last legacy source. |
...
program to 27 slices; the R3a/R3b/R3c split brought it to 29; and the later
R3c1–R3c3 split brings it to exactly 31. R5a requires an extension-API update
...
| `NativeToolCapabilities`: `ToolCapabilityState` pointer including `active_tool_names` | state and request/execution snapshots | candidate `publish()`; class-A `set_active_tools()` | Base locking and prepare/publish APIs landed; R3a detached build; R3c1 consumes/type-aligns the existing value without editing this owner; R3c3/R4b integration; R5b admission |
| Guarded provider selection owner: `NativeReplProviderState.selection`, `thinking_level`, and pending-default payload/state | selection/provider/thinking/request/reload/persistence-payload reads | selection/auth/reset/startup/cycle/RPC/reload and class-A thinking/model commits | R3c1 adds detached selection/fallback/default preparation and assignment-only publication; R3c3 publishes reload preparation. `_ProviderMutationEffects` may orchestrate only those detached owner APIs before acceptance. R5b/R6 retain class-A thinking/model/provider admission and durable default handling; R5b keeps `mutation_io_lock → session mutex` for thinking commit/order and releases the session mutex before durable tree I/O |
...
These bounds support the exact 31-slice plan. “Named mechanisms” may list
...
| R3c1 — **next** | Real provider-catalog, coding-session, and `NativeReplProviderState` selection/pending-default detached prepare/non-fallible publish owners; consume/type-align existing `NativeToolCapabilities` APIs; `PreparedReloadEffects` alignment. Exact sources: `catalog_state.py`, `coding/state.py`, `repl_state.py`, and `session_generation.py`; exact editable tests: `tests/test_native_catalog_state.py`, `tests/test_native_coding_state.py`, `tests/test_native_repl_state.py`, and `tests/test_native_session_extension_generation.py`; four planning docs only. | Behavior-neutral snapshot/no-alias preparation, failure isolation/disposal, assignment-only publication, no provider factory during publish, and recursive no-caller inventory. `_ProviderMutationEffects` may only orchestrate detached preparation in R3c3, with no pre-acceptance live mutation/rollback. Run `tests/test_native_tool_capabilities.py`, `tests/test_native_extension_providers.py`, `tests/test_native_provider_catalog.py`, and `tests/test_native_dynamic_provider_swap.py`; `just check` is the full-suite gate; 1,200 production+test changed lines, 400/source. |
...
| R3c3 | Startup/reload installation of R3a/R3b through R3c1 owner APIs and R3c2 routing seam; exact existing four-source/four-test composition manifest plus architecture/spec/backlog/extension API/changelog. | Exact order: activation → R3a builder → installed-route replacement `session_start` once → provider catalog/factory/refresh/fallback → coding/history/usage/compaction → unavailable/default/capability → one freeze → chrome final acceptance → gate reserve/install plus assignment-only publication under session mutex → unlock → frozen staged delivery → gate release/drain → presentation/persistence. Static instrumentation permits only gate state, assignments, and retired pinning under the mutex and forbids candidate↔session, session→chrome/`mutation_io_lock`, and chrome→paint edges. No undo log, compensating provider rollback, duplicate lifecycle, mutable prepared holder, or destructive reconcile/repair hunk. Preserve two deltas, every R3a arm, and 1,200/400 limits; update the existing `### Fixed` bullet beginning “Extension reload no longer clears live retained TUI chrome before activation”. |
