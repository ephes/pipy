...
the R3a/R3b values and invokes the R3c1a–R3c1c owner APIs. R4b binds tool
advertisement/execution, tool and custom renderer selection, and provider
contribution consumers to published generation snapshots. D1b0 now admits live
provider execution through the state-owned guarded run binding. R4c now binds
menu/lifecycle/chrome consumers to that same snapshot and closes displaced
chrome handles outside the session mutex. R5a now ships terminal coding-effect
and sidecar invocation plus guarded tree/input durable ordering. R5b now ships
generation-bound stale/gated/terminal refusal for active-tool/thinking controls,
guarded provider selection/thinking access, and ordered durable thinking append.
R6 now ships generation-bound atomic `set_model` admission, unlocked provider
preparation, non-fallible selection/coding commit, and post-lock fail-soft
presentation/default persistence.
The
...
R3c3 owner freshness, not R5b's active-tool/thinking or R6's now-shipped model
generation-bound class-A API conversion.
...
validation and publication. R3c2 routing, R3c3 composition, R4a command/request/session-gate snapshots
plus live queue synchronization, R4b tool/renderer snapshots and provider contribution consumers,
...
| `SessionExtensionGeneration` contains runtime plus flags while capability, renderer, lifecycle, provider, menu, and chrome projections publish separately. | **Landed through R4c.** `build_extension_projection()` constructs the detached immutable runtime/flag, command/menu/shortcut, hook, tool/capability, renderer, provider, queue-handle, and exact-chrome families. R4a–R4c moved every production consumer before deleting its corresponding legacy source and equivalence arm. The final separately stored generation flag map, temporary lifecycle/flag reload effect, mutable emitter copies, and direct runtime menu/lifecycle reads are gone. |
| Production operations read `current` per access rather than one snapshot. | **Landed through R4c.** R4a covers command/request/session-gate and renderer drain, R4b covers tool/renderer iterations and provider contribution consumers; D1b0 guards live provider admission through coding state, and R4c covers menu/lifecycle/chrome plus user-bash inputs. Barrier tests prove each operation observes one old or new generation across flags, tools, renderers, hooks, providers, menu, and chrome. Direct custom tree/render/input delivery remains R1-compatible and does not consult routing; only coherent drain consultation affects queue side effects. |
...
| `_CommandContext.set_model()` | `_ProviderMutationEffects.extension_set_model()` | `NativeReplProviderState.selection` plus `CodingSessionState` provider binding/history/usage | R6 — shipped |
...
readers and writers. R5a serializes each complete retained coding-session
effect with the shared tree/input owners and makes every retained effectful
...
but cannot mutate it. R5b makes terminal, stale, or publication-pending `set_active_tools` and
`set_thinking_level` return `False`; R6 applies the same terminal/stale/gated
`False` contract to `set_model`. Ordinary stale-generation admission remains
exactly those three
...
to 31; and splitting R3c1 into R3c1a–R3c1c brings it to exactly 33. R5a ships the extension-API update
and changelog fix entry for preventing concurrent/reordered and post-run
provider/session-tree/persisted-entry/input-queue effects. A future
...
| --- | --- | --- |
| C01 | One per-run session mutex guards the generation pointer and publication gate. | **Landed through R6 for every class-A consumer.** `NativeToolReplSession.run()` passes one exact `threading.RLock` to `SessionGenerationRef`, routing/gate state, coding state, provider selection/thinking, and tool capabilities. Both production projection builders reject queue/reference mutex mismatch, and reload's owner checks plus every session publication write execute under one acquisition of that mutex. Candidate-host publication remains deliberately outside it, so candidate and session guards never nest. |
| C02 | Publication is a non-fallible pointer operation and does not release the displaced generation under the mutex. | **Landed through R4c.** After unlocked candidate-host prepublication, one uninterrupted mutex section checks every expected owner before its first write, then publishes the generation/routing pointer and vetted non-fallible owner assignments. The preallocated displaced-owner inventory stays referenced until unlock. A post-prepublication mismatch performs no session publication write; caller cleanup retires the candidate route and closes candidate chrome outside the mutex. |
| C03 | Every reader and writer of guarded state takes the same guard; no check-then-mutate occurs, and no provider/filesystem I/O runs under the session mutex. | **Landed through R6 for the complete guarded set.** Coding-state and capability pointer access already use the session mutex (`tests/test_native_coding_state.py`, `tests/test_native_tool_capabilities.py`), and R1 now guards every activation-host reader/writer plus its atomic failure decision, seal/freeze, and disposal. R2 owns the sink-local closed-check/write/attach/close guard and the narrow replacement-`session_start` candidate routing seam; R4c now binds ordinary and retained invocations to a generation, selects the exact displaced handle, and closes it after session unlock. Generation-outbox append/drain/close and all live `NativeReplProviderState` selection/thinking readers and writers share the session mutex; R6 model admission checks and commits there while provider/default I/O remains unlocked. R5a's one coordinator now owns retained-effect admission/in-flight state, active-tree mutation/order, and every coding-input reader/writer. Provider/render work runs outside `mutation_io_lock`; durable tree I/O deliberately holds that lock but remains outside the session mutex. The guarded-field table below is exhaustive for the target. |
| C04 | A generation has one immutable contribution/configuration projection. | **Landed through R4c for every production consumer.** Both production paths call the same R3a builder, and `SessionExtensionGeneration` carries that exact frozen `ExtensionProjection` through the single `SessionGenerationRef` pointer. R4b removes the temporary renderer publication and direct legacy tool/renderer/provider sources only after their final consumers move; R4c does the same for the final flag/menu/lifecycle sources and arms. |
| C05 | Operations take one generation snapshot at start and keep it for their duration. | **Landed through R4c for command/request/session-gate/outbox-drain, tool/renderer and provider contribution, menu/lifecycle/user-bash, and chrome operations.** `_SessionExtensionOperations` preserves the R4a flag and identity contracts. `_SessionExecutionProjections` takes one mutex snapshot per provider iteration and retains the projected registry/executor, tool-call hooks/flags, filtered renderer map. Tool workers and renderer pending state retain those values across reload. D1b0 admits live provider execution through the state-owned guarded run binding; replacement invalidates the active run rather than continuing it across reload. Custom message/entry render, replay, redraw, append, and direct-send rendering select one projected map per operation. Startup and refusal provider contribution/refresh paths use one projected tuple. No provider construction, execution, rendering, or callback occurs under the session mutex. The R4a drain method remains unchanged. |
| C06 | Class-A contexts capture their creating generation; liveness/gate check and in-memory mutation are one critical section and fail closed. | **Landed through R6.** Retained operation contexts and extension-tool ports capture one `generation_id`; `_ProviderMutationEffects` compares it with terminal/gate state and commits tools, thinking, or a fully prepared model mutation under the shared mutex. Denied mid-turn model contexts remain generation-bound and return `False`. |
| C07 | The publication gate opens before derived live selection is sampled, remains open through publication, closes on every failure, and is closed before replacement lifecycle hooks. | **Gate foundation landed; hook ordering superseded by R3c3's first documented delta.** Configuration/resource reload and candidate activation run outside the gate. R3c3 then installs candidate routing and invokes replacement `session_start` before opening the first `generation_ref.publishing()` section, which covers all derived live-state preparation and the pointer swap, then remains open through accepted staged-message delivery, both route-release phases, and gate drain. The session mutex is normally unlocked for those post-commit paths, but `publication_pending` stays true and extension-visible sinks may run until drain completes; the section still closes on every exit. If that section refuses the candidate—including `ReloadPreparationRefused` because `auth_store` is absent—`_ReloadCommandEffects.execute()` opens a second, separate `publishing()` section around the ordinary retained `refresh_provider_after_reload()` and `_diagnose_unknown_tool_filters()` path; an absent auth owner makes the refresh routine return before mutation, but does not bypass either call. Explicitly assigning a valid `AuthStore` later lets a subsequent reload prepare again; candidate preparation/publication never proceeds without that owner. Presentation/persistence remains outside both sections. A refusal after the hook discards candidate chrome and staged messages but may retain immediate non-staged, non-chrome lifecycle effects such as `notify`. R5b active-tool/thinking admission does not mistake unlocked host prepublication for generation acceptance or assume one uninterrupted gate: a class-A mutation may be admitted between the two publishing sections, so each section is checked independently. The publication-gate tests pin gate closure and pointer-swap coverage; R6 applies the same independent-gate recheck to model admission. |
...
| C20 | Active provider, thinking level, and active tools are generation-scoped mutable selection, rebased without losing a mutation accepted before cutover. | **Landed through R6.** `prepare_reload_state()` itself captures expected live selection/pending-default while its caller briefly holds the shared session mutex; the caller supplies only replacements. R3c3's comparison and publication later share one uninterrupted mutex section. `snapshot_reload_state()` and any retained-selection/default refresh snapshot path are absent and never existed in the committed baseline. The publisher writes replacement selection/default only and never restores `thinking_level`; `test_repl_reload_publication_preserves_thinking_changed_after_preparation` proves a later accepted thinking change remains live. This owner freshness does not alter R5b's active-tool/thinking or R6's model generation-bound admission. |
| C21 | Provider catalog contribution and coding provider binding publish coherently; refresh preserves history/usage, fallback replaces history/usage, and compaction survives. | **Landed through R6.** R3c3 checks the expected binding under the shared mutex immediately before acceptance and refuses a mismatch without invoking a publisher. Refresh publishes replacement binding only and usage publication is a no-op. Fallback publishes replacement binding plus immutable empty replacement history and swaps in one owner-built cleared usage accumulator. R4b makes startup and refusal refresh consume a single projected provider tuple while D1b0 admits each live provider execution through the coding state's run witness and captured binding. A binding replacement invalidates the active run; no live provider continuation across reload is promised. R6 model switching keeps the established rebind contract: provider/selection move together, history and usage clear, and compaction/provider failure survive. Neither reload nor model mutation constructs a provider under the session mutex. |
| C22 | Coding provider binding, canonical history, usage accumulator, and compaction state use the session mutex on every reader/writer. | **Landed.** `CodingSessionState` guards binding, messages, usage and compaction APIs; blocking-reader/writer tests in `tests/test_native_coding_state.py` cover rebind, snapshots, append, and compaction metadata. R6 reuses this owner and adds no second lock. |
| C23 | Active-tool capability state uses immutable candidate values and the session mutex. | **Landed through R5b.** One `NativeToolCapabilitySnapshot` binds the current live active selection to the projected registry/executor under the shared mutex. Execution carries that snapshot's generation id into extension-tool contexts; same-generation additive visibility remains supported, while stale/gated/terminal mutation refuses and a reload's distinct executor cannot leak successor tools into the old call. |
| C24 | Renderer selection remains pinned to the tool call that advertised it. | **Landed through R4b.** Each provider iteration selects the projected renderer map paired with its advertised tool snapshot; both captured and TUI renderers retain the resolved renderer from call through result. Custom message/entry render operations likewise retain one projected map. The R4a outbox drain method is unchanged. |
| C25 | Runtime plus parsed flags reject as one candidate. | **Landed for current runtime/flags; R3a adds only failure-isolated detached construction.** Slice 47 retired the former `_reload_extension_generation()` split owner: startup and reload now enter `repl/extension_attach.py::attach_generation()`, while the reload driver still supplies activation plus parsed flags as one candidate. Builder-step failures return no projection and cannot change the retained live reference or adapter. |
| C26 | Teardown invalidates class-A and coding-session mutation liveness and closes queue/chrome sidecars for a worker that outlives `run()`. | **Landed through R6.** `CodingSessionController.run_loop()` preserves settle then `session_shutdown`; a nested teardown `finally` atomically closes admission under `mutation_io_lock`, then waits on that lock's condition for every admitted lease (the wait releases the lock). Once quiescent, it briefly takes the session mutex while holding `mutation_io_lock` to invalidate/detach the generation and close/detach its generation queue. No I/O occurs under the session mutex. After both release, chrome closes under its sink guard and paint/disposal follows. Calls ordered before terminal finish in coordinator order; later effectful coding-session calls raise `ExtensionCapabilityError`, while all three class-A families return `False`. A model candidate still preparing when terminal wins its commit recheck refuses, and a callable first released after teardown refuses before provider construction. |
...
candidate chrome, R4c extends the entry to closed retired-live handles, and R5a
...
share the session mutex, detecting even refused publication windows. Compaction
captures and compares these identities under outer tree/effect lock then session
...
host publication or any active-generation write. Static inventory pins three
`mark_retired_locked()` and three `finalize_retirement()` production calls
...
the mutex only in the extension port would remain one-sided. R5b removed direct thinking access from `_ProviderMutationEffects`, the
session-thread cycle, footer/settings readers, and
`automation.rpc.NativeRpcServer._set_thinking_level`; those adapters now use
`NativeReplProviderState` methods guarded by the exact session mutex. Production
direct live-owner accesses remain only inside R3c3's caller-held publication
section; detached reload-shadow fields are not shared. R6 preserves that owner
and completes atomic model admission with caller-held expected-state checks and
assignment-only publishers. R5b/R6 tests enumerate converted
selection reader/writer methods and fail if a new direct access appears. R5a inventory
...
7. **Model/class-A commit:** R6 first claims the coordinator effect lease. A
   short `mutation_io_lock → session mutex` section refuses terminal/stale/gated
   calls and captures exact REPL and coding expectations, then releases both
   locks. Catalog resolution, provider construction, capability inspection, and
   detached coding replacements run unlocked from the session mutex. A second
   section in the same lock order atomically rechecks terminal, generation,
   gate, exact selection/thinking/pending-default values, and coding-binding
   identity before assignment-only selection/binding/empty-history/cleared-usage
   publication. Both locks release before footer refresh or defaults I/O. The
   logical effect lease remains until those fail-soft post-effects finish, so
   terminal teardown cannot finish through a winning model commit. If terminal
   wins while preparation is running, the commit refuses and teardown waits only
   for the losing lease to leave. A post-terminal call refuses before provider
   construction.
8. **Terminal:** after settle then `session_shutdown`, a nested teardown
...
R4a, R4c, R5a, R5b, and R6 instrument their relevant edge/refusal boundary.
...
| R3a — **shipped** | Standalone `session_generation` projection values/builder and pure projected candidate-composition adapter; no settings adapter and no R1 `activation_hosts` ownership state. | R3a shipped detached immutable construction, copied mappings/tuples, shallow-frozen custom-message options, private port flag dictionaries, queue/reference mutex validation, exact R2 chrome identity, per-family equivalence, and no-alias tests. R3c3 installs the exact projection; R4a–R4c consume every production family and delete each proven obsolete source/equivalence arm only after its last consumer moves. |
...
| R4b — **shipped** | Tool advertisement/execution, renderer **selection/rendering other than the R4a drain method**, provider contribution/refresh consumers, snapshot adapters, and proven legacy-source deletion. | One session-mutex snapshot binds each provider iteration's projected registry/executor, tool-call hooks/flags and filtered renderer map; custom render operations bind one projected map. D1b0 uses the state-owned guarded run binding for live provider admission, with no provider field retained in the tool/renderer projection. Startup/refusal provider consumers use projected tuples. R0 selection/rebinding remains; provider construction stays unlocked; converted legacy sources and only their tool/renderer/provider equivalence arms are deleted. No changelog entry applies because R4b consumes R3c3's published coherence. |
| R4c — **shipped** | Menu/description/shortcut, lifecycle and user-bash inputs, generation-bound retained chrome reads/writes, retired close, and reload composition. Terminal invocation landed in R5a. | One generation snapshot per operation. Publication captures the displaced handle under the session mutex; after unlock its sink-local guard performs closed-check/snapshot/detach, and callbacks/disposal/paint/last-reference release run after all guards. The separately stored flag map, temporary lifecycle/flag reload effect, mutable emitter copies, direct runtime contribution reads, and final equivalence arms are gone. |
| R5a — **shipped** | One coding-effect coordinator in the composition root; effectful `ExtensionCodingSessionControl` adapters; active `_RunControlState.session_tree` pointer access/rebinds; `NativeSessionTree` mutable owner and durable writer; all `CodingInputQueue` readers/writers and session-controller/agent/RPC adapters; `CodingSessionController.run_loop()` finally and `NativeToolReplSession.run()` terminal ports; terminal generation-queue/chrome invocation. Provider/selection mutation is excluded except terminal invalidation. | Promote existing `mutation_io_lock` plus a condition on it into one coordinator. An exclusive/reentrant owner lease serializes effects while the lock is released; shared tree/input phases use the lock; tree append alone holds it through durable I/O. Terminal condition-waits, then uses `mutation_io_lock → session mutex`; chrome/paint follow after release. |
| R5b — **shipped** | Only class-A `set_active_tools`/`set_thinking_level`; guarded selection owner; session-thread cycle and RPC thinking adapters; generation admission and post-lock tree/footer adapters. Coding-session callable design and terminal wiring are already R5a; `set_model` is excluded. | Existing session mutex for generation-bound active-tool/thinking admission. Thinking uses `mutation_io_lock → session mutex`, releases the inner lock before tree I/O and the outer lock before footer paint, and has no reverse edge. |
| R6 — **shipped** | `set_model` port including terminal refusal, provider/catalog preparation adapter, guarded provider/coding selection owner, and post-lock persistence/presentation adapter. | Three-phase model mutation: an R5a effect lease plus initial `mutation_io_lock → session mutex` admission/capture; fallible provider/catalog/coding preparation outside the session mutex; the same lock order for atomic terminal/generation/gate/expected-owner recheck and non-fallible in-memory assignment; then unlocked fail-soft presentation/default persistence. Deterministic barriers pin stale, gate, terminal, newer-selection, construction-failure, persistence-failure, success/current-turn, and retained-after-terminal outcomes. |
...
session mutex is normally unlocked; R5b active-tool/thinking controls account
for extension-visible code running with `publication_pending` true there, and R6
does the same for model. On a mismatch the caller
...
and persistence run after it closes. R5b now admits tools/thinking against the
current generation id plus each gate section independently; R6 preserves that
rule for model and does not treat the
...
| Closed rejected/retired sidecar drops appends/chrome writes; retired queue handles remain observably undeliverable as today; worker outliving `run()` mutates no class-A or coding-session state | **Landed through R6.** Retained post-terminal model calls cannot construct/publish a provider, rebind coding state, refresh presentation, or persist defaults. |
...
| Rejected listeners/chrome requests; post-seal contribution; normal published handler queue/chrome use | Post-seal contribution refusal **landed in R1**, rejected candidate chrome/listener close **landed in R2**, queue conversion **landed in R4a**, and generation-bound retired-live chrome **landed in R4c**. |
| Candidate flag failure keeps runtime/flags | **Landed**, including R2 preservation of old retained chrome/listeners, R3c3 installation, and complete R4a–R4c bounded snapshot consumption. |
| Provider fallback and extension-provider refresh preserve their established history/usage/compaction behavior | **Landed through R6.** Reload refresh/fallback characterization remains unchanged; model rebind clears history/usage and retains compaction/provider failure. |
| Active-tool filtering and renderer pinning stay compatible | **Landed through R5b**, including generation-bound active-tool admission. |
| Lifecycle ordering stays compatible | **Landed in R4c**, using the existing post-gate order as characterization. |
| Removed/disabled chrome clears only after successful commit | **Landed through R4c**: R2 owns acceptance timing and R4c owns coherent generation binding/retired-live closure. |
| Concurrent coding history/usage updates are not lost | **Landed through R6.** Exact binding identity prevents a prepared model from overwriting a newer coding-provider publication; the accepted model rebind clears then-current history/usage atomically by contract. |
...
| Coherent published projection | The new success integration test observes one `new` generation across real command, hook, flag, tool, and renderer consumers; `test_r4b_tools_and_renderers_stay_pinned_while_provider_replacement_invalidates_run`, the menu test, and the chrome PTY success test cover tool/renderer iteration with guarded provider invalidation, menu/shortcut, and retained UI composition. |
| One immutable generation snapshot per operation | `test_r4a_dispatch_uses_one_old_or_new_published_snapshot`, `test_r4b_tools_and_renderers_stay_pinned_while_provider_replacement_invalidates_run`, `test_custom_message_rendering_keeps_one_generation_snapshot`, and `test_agent_event_adapter_snapshots_hooks_flags_and_chrome_context_once` cover dispatch, tool/renderer snapshots with guarded provider invalidation, custom-render, and lifecycle/chrome operations. |
