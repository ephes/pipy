...
Status: R0-reconciled completion contract; R1–R5a are shipped and R3/R4 are
complete; R5b is active/next. The original
...
and sidecar invocation plus guarded tree/input durable ordering. Production still
lacks generation-bound stale mutation refusal and atomic `set_model` admission;
R5b–R6 retain those owners.
The
...
and R4c menu/lifecycle/chrome snapshot consumers are shipped; R5a now ships the
coding-effect coordinator and terminal teardown, and R5b is active/next.
...
| Class-A mutation ports do not capture or validate `generation_id`. | **R5a terminal-generation support and teardown landed; remaining admission is required in R5b–R6.** R5b owns `set_active_tools` and `set_thinking_level`; R6 owns only `set_model`. |
...
| C23 | Active-tool capability state uses immutable candidate values and the session mutex. | **R3a detached build, R3c3 publication, and R4b request/execution snapshots landed; R5b retains generation-bound class-A admission.** One `NativeToolCapabilitySnapshot` binds the current live active selection to the projected registry/executor under the shared mutex. Execution retains that immutable value; a same-generation extension tool can still announce additive visibility, while a reload's distinct executor cannot leak successor tools into the old call. |
...
| C26 | Teardown invalidates class-A and coding-session mutation liveness and closes queue/chrome sidecars for a worker that outlives `run()`. | **R5a landed coding-session invalidation and terminal queue/chrome invocation; class-A refusal remains required in R5b/R6.** `CodingSessionController.run_loop()` preserves settle then `session_shutdown`; a nested teardown `finally` atomically closes admission under `mutation_io_lock`, then waits on that lock's condition for every admitted lease (the wait releases the lock). Once quiescent, it briefly takes the session mutex while holding `mutation_io_lock` to invalidate/detach the generation and close/detach its generation queue. No I/O occurs under the session mutex. After both release, chrome closes under its sink guard and paint/disposal follows. Calls admitted before terminal finish in coordinator order; later effectful coding-session calls raise `ExtensionCapabilityError`. R5b makes active-tool/thinking return `False`; R6 does so for `set_model`. The R5a API/changelog update is shipped. |
...
These bounds support the exact 33-slice plan. The shipped prefix now ends at R5a, and the mandatory remaining order begins
with R5b. “Named mechanisms” may list
...
| Closed rejected/retired sidecar drops appends/chrome writes; retired queue handles remain observably undeliverable as today; worker outliving `run()` mutates no class-A or coding-session state | **Queue closure landed in R4a, retired chrome closure in R4c, and terminal invocation/coding-session refusal in R5a; class-A refusal remains required in R5b/R6.** |
...
| Active-tool filtering and renderer pinning stay compatible | R4b snapshot and pinning **landed**; generation-bound class-A admission remains R5b. |
