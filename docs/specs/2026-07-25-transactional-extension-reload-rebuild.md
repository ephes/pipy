...
turn/contribution consumers to one published generation snapshot. R4c now binds
...
| C05 | Operations take one generation snapshot at start and keep it for their duration. | **Landed through R4c for command/request/session-gate/outbox-drain, tool/renderer/provider, menu/lifecycle/user-bash, and chrome operations.** `_SessionExtensionOperations` preserves the R4a flag and identity contracts. `_SessionExecutionProjections` takes one mutex snapshot per provider iteration and retains the projected registry/executor, tool-call hooks/flags, filtered renderer map, and current already-built coding provider. Tool workers and renderer pending state retain that value across reload; the next iteration snapshots the successor. Custom message/entry render, replay, redraw, append, and direct-send rendering select one projected map per operation. Startup and refusal provider contribution/refresh paths use one projected tuple. No provider construction, execution, rendering, or callback occurs under the session mutex. The R4a drain method remains unchanged. |
...
| Installed immutable `ExtensionProjection` in `SessionExtensionGeneration` | R4a command/request/session-gate and renderer-drain snapshots, R4b tool/renderer/provider snapshots, and R4c menu/lifecycle/chrome snapshots | R3a pure builder; R3c3 startup/reload installation | R3a construction, R3c3 installation, and R4a–R4c bounded consumer migration landed; all production consumers use the projection |
...
| R4b — **shipped** | Tool advertisement/execution, renderer **selection/rendering other than the R4a drain method**, provider contribution/refresh consumers, snapshot adapters, and proven legacy-source deletion. | One session-mutex snapshot binds each provider iteration's projected registry/executor, tool-call hooks/flags, filtered renderer map, and current preconstructed coding provider; custom render operations bind one projected map. Startup/refusal provider consumers use projected tuples. R0 selection/rebinding remains; provider construction stays unlocked; converted legacy sources and only their tool/renderer/provider equivalence arms are deleted. No changelog entry applies because R4b consumes R3c3's published coherence. |
