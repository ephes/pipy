...
Status: R0-reconciled completion contract; R1–R5b are shipped and R3/R4 are
complete; R6 is active/next. The original
...
Production still lacks atomic `set_model` admission; R6 retains that owner.
The
...
R3c3 owner freshness, not R5b's now-shipped active-tool/thinking or R6's pending
model generation-bound class-A API conversion.
...
admission, and R6 is active/next.
...
| Class-A mutation ports do not capture or validate `generation_id`. | **Landed through R5b for `set_active_tools` and `set_thinking_level`; R6 remains.** Retained contexts and extension-tool ports now bind those two controls to their creating generation and refuse stale, gated, or terminal calls. R6 owns only `set_model`. |
...
but cannot mutate it. R5b now makes terminal, stale, or publication-pending
`set_active_tools` and `set_thinking_level` return `False`; R6 applies
terminal/stale `False` to
...
| --- | --- | --- |
| C01 | One per-run session mutex guards the generation pointer and publication gate. | **Landed through R5b for active-tool/thinking consumers; R6 retains model adoption.** `NativeToolReplSession.run()` passes one exact `threading.RLock` to `SessionGenerationRef`, routing/gate state, coding state, provider selection/thinking, and tool capabilities. Both production projection builders reject queue/reference mutex mismatch, and reload's owner checks plus every session publication write execute under one acquisition of that mutex. Candidate-host publication remains deliberately outside it, so candidate and session guards never nest. |
...
| C26 | Teardown invalidates class-A and coding-session mutation liveness and closes queue/chrome sidecars for a worker that outlives `run()`. | **Landed through R5b for coding-session and active-tool/thinking refusal; R6 retains model refusal.** `CodingSessionController.run_loop()` preserves settle then `session_shutdown`; a nested teardown `finally` atomically closes admission under `mutation_io_lock`, then waits on that lock's condition for every admitted lease (the wait releases the lock). Once quiescent, it briefly takes the session mutex while holding `mutation_io_lock` to invalidate/detach the generation and close/detach its generation queue. No I/O occurs under the session mutex. After both release, chrome closes under its sink guard and paint/disposal follows. Calls admitted before terminal finish in coordinator order; later effectful coding-session calls raise `ExtensionCapabilityError`, while active-tool/thinking return `False`; R6 does the same for `set_model`. |
...
section; detached reload-shadow fields are not shared. R6 must preserve the
owner and complete atomic model admission. R5b tests enumerate converted
...
These bounds support the exact 33-slice plan. The shipped prefix now ends at R5b, and the mandatory remaining order begins
with R6. “Named mechanisms” may list
...
must do the same for model. On a mismatch the caller
...
current generation id plus each gate section independently; R6 must preserve
that rule for model and may not treat the
...
| Stale model and hook model controls; stale thinking; stale active tools; pending-publication refusal; accepted-before-gate survival | **Landed in R5b for thinking/tools**, including terminal refusal and admitted-before-gate survival; model/hook model remain R6. |
...
| Closed rejected/retired sidecar drops appends/chrome writes; retired queue handles remain observably undeliverable as today; worker outliving `run()` mutates no class-A or coding-session state | **Landed through R5b for queues, chrome, coding-session, active-tool, and thinking refusal; `set_model` remains R6.** |
...
| Provider fallback and extension-provider refresh preserve their established history/usage/compaction behavior | Behavior characterization, coherent publication, and R4b provider snapshot consumption **landed**; R6 retains model admission. |
