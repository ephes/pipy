...
Status: R0-reconciled completion contract; R1 is shipped and R2 is next. The
original Slice 3 text remains historical design evidence, while the 2026-07-30
table below controls bounded R1–R7 completion.
...
R1 has since added the isolated, guarded, sealed/disposed activation host. The
implementation still lacks complete frozen generation projections, production
operation snapshots, generation-bound stale mutation refusal, and atomic
`set_model` admission. Rejected activation can also clear retained chrome
because candidate chrome still uses the live host; that remains R2-owned.
...
| --- | --- |
| Candidate activation uses the live host and rejection can clear prior chrome. | **R1 registration-host portion landed; R2 chrome portion required.** R1 replaced the activation registration host and disposes rejected candidates. R2 must stage generation chrome/listener requests and preserve live chrome until acceptance. Evidence of the remaining gap: `_ReloadCommandEffects._reload_extension_generation()` calls `clear_extension_chrome()` before `_activate_workspace_extensions()` and flag parsing. |
...
no-ops. The host guard is released before frozen messages are appended/flushed
under the session mutex, so the two guards never nest. The flush and accepted
live runtime appends share that section so staged user
messages remain first. Sink delivery and
detached-value release happen after unlock. Every extension-facing
...
| R4a | Command/shortcut/input/request/session-hook consumers and snapshot adapter; every generation-outbox writer (`_ActivationApi.send_user_message()`, `send_message()`/`sendMessage()`, `_commit_activation()` staged user-message flush); `_CustomEntryRenderer.drain_extension_outboxes()` **only** (not rendering, accepted staged-custom-message delivery, or coding-session sends); queue rejection/retirement close; provider-header request-local trust capture; reload composition only for those close/deletion duties; closed-queue API documentation. | Candidate host guard only to detach staged messages, then release; existing session mutex as the queue guard for staged-first flush versus runtime append, closed-check+append, atomic detach/drain, close, and one operation snapshot. No nested edge. |
...
| Candidate user/custom outboxes are not delivered on rejection | Activation staging and R1 sealed/disposed abandonment are **landed**; R3/R4a still own closed sidecars and synchronized live delivery. |
...
| Rejected listeners/chrome requests; post-seal contribution; normal published handler queue/chrome use | Post-seal contribution refusal **landed in R1**; listeners/chrome and published queue/chrome use remain **required in R2/R4a/R4c**. |
