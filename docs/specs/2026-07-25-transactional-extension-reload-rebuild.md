...
Status: R0-reconciled completion contract; docs/spec decision complete and R1
is next. The original Slice 3 text remains historical design evidence, while the
...
list operations is erased. This is not the joined `pipy-ext-activate` path. R3
now owns a queue-sidecar value and R4a owns every writer of those two outboxes:
the named activation-api methods and the successful-activation **user-message**
flush, plus the named renderer drain, under the existing session mutex. Staged
activation custom messages are instead harvested into
`ActivatedExtension.custom_messages`; initial/reload composition calls
`_CustomEntryRenderer.extension_send_message()` directly after acceptance and
...
activation-timeout path. Nevertheless, an activated tool can retain its old
`api` and call `register_*` or `on` after activation today. `_activated` affects
only message routing: contribution methods neither check it nor have a seal, so
they return normally while mutating the already-harvested host's `_staged*`,
`_hooks`, `_failure`, or `_flag_values` state. The detached command/tool/hook/
provider/renderer tuples and maps do not gain the late contribution; a late
flag default can still mutate the host mapping shared by harvested
`RegisteredFlag` objects, while the separately parsed generation flag mapping
remains detached. A `pipy-tool-call` cannot race its own initial harvest because
that tool is not runnable until after harvest/publication, but extension-created
activation threads can race today's multiple unguarded `staged_*` reads. R1 is
therefore present-day API correctness—late registration currently appears to
succeed into dead or partially shared host state—as well as future timeout
safety. It must atomically seal/snapshot a successful host, dispose every
rejected or otherwise abandoned host, and refuse all later class-D writes by
raising `ExtensionCapabilityError` at the late `register_*`,
`unregister_provider`, or `on` call boundary. That single refusal shape applies
to void-return registrations, direct `on(event, handler)`, decorator-form
`on(event)`, and `register_flag`; R1 must not fabricate an inert decorator or
`RegisteredFlag`. R1 does not add timeout policy; its `docs/extension-api.md`
and changelog fix text say retained activation APIs raise after activation or
rejection and make no timeout claim.
...
| --- | --- |
| Candidate activation uses the live host and rejection can clear prior chrome. | **Required in R1–R2.** R1 replaces the activation registration host; R2 stages generation chrome/listener requests and preserves the live chrome until acceptance. Evidence of the current gap: `_ReloadCommandEffects._reload_extension_generation()` calls `clear_extension_chrome()` before `_activate_workspace_extensions()` and flag parsing. |
| An abandoned activation host is not sealed from later registration. | **Required in R1.** `_ActivationApi` has mutable staged registries/contributions, `_failure`, `_flag_values`, staged messages, and `_activated`, but no guard, sealed flag, or disposed state. A retained post-activation API currently accepts class-D calls into already-harvested host state, so R1 is a current correctness fix, not only timeout preparation. R1 owns only the activation/registration-host and reload-composition adapter family. |
| `SessionExtensionGeneration` contains runtime plus flags while capability, renderer, lifecycle, provider, menu, and chrome projections publish separately. | **Required in R3 and R4a–R4c.** R3 builds the complete value; R4a moves command/request/queue consumers, R4b tool/renderer/provider consumers, and R4c menu/lifecycle/chrome consumers and deletes each legacy source only after its last consumer moves. |
...
| --- | --- | --- |
| C01 | One per-run session mutex guards the generation pointer and publication gate. | **Landed.** `NativeToolReplSession.run()` creates one `threading.RLock` and passes it to `SessionGenerationRef`, `NativeToolCapabilities`, `SettingsManager`, `KeybindingsManager`, and `CodingSessionState`. `test_capabilities_and_the_generation_share_one_session_mutex`, `test_a_caller_supplied_settings_manager_adopts_the_session_mutex`, `test_a_caller_supplied_manager_adopts_the_session_mutex` (keybindings), and `test_coding_state_shares_the_session_mutex_when_bound` pin the shared identity. |
| C02 | Publication is a non-fallible pointer operation and does not release the displaced generation under the mutex. | **Landed for the pointer; required in R3/R4c for the complete projection.** `SessionGenerationRef.publish()` assigns the pointer/id and returns the retired value; `test_generation_ref_publishes_a_new_identity_and_returns_the_retired_value` pins it. R4c must make this the sole semantic extension commit. |
| C03 | Every reader and writer of guarded state takes the same guard; no check-then-mutate occurs, and no provider/filesystem I/O runs under the session mutex. | **Required in R1/R2/R4a/R4c/R5a/R5b/R6.** Coding-state and capability pointer access already use the session mutex (`tests/test_native_coding_state.py`, `tests/test_native_tool_capabilities.py`), but candidate host harvest/registration and generation-outbox append/drain are unsynchronized, chrome is generation-unbound, `NativeReplProviderState` has bare selection/thinking fields, and `set_model` admission is split. In addition, `NativeSessionTree` releases `_write_lock` before `_write_entry()` and `CodingInputQueue` is wholly unguarded while a retained coding-session control is concurrently reachable. R1 owns the candidate guard; R4a owns the session-mutex generation queue; R2/R4c own chrome guards; R5a makes one coordinator own retained-effect admission/in-flight state, active-tree mutation/order, and every coding-input reader/writer. Provider/render work runs outside `mutation_io_lock`; durable tree I/O deliberately holds that lock but remains outside the session mutex. The guarded-field table below is exhaustive for the target. |
| C04 | A generation has one immutable contribution/configuration projection. | **Required in R3.** Current `SessionExtensionGeneration` has only `_ExtensionRuntime` plus mutable `dict` flag values; reload separately calls renderer, capability, catalog, emitter, menu, and TUI owners. R3 builds immutable runtime/flags, command/menu/shortcut, hook, tool/capability, renderer, provider, queue-handle, and chrome-handle projections before publication. |
...
| C08 | Candidate contribution registration is isolated, guarded, one-way sealed before freeze, and sealed/disposed on rejection or abandonment. | **Required in R1; not narrowed.** `_ActivationApi` stages commands, shortcuts, hooks, tools, providers/unregistrations, flags/value storage, message/entry renderers, user/custom messages, failure, and `_activated`, but has no guard or seal. Post-activation `register_*`/`on` currently mutates already-harvested host state and returns as though accepted; a tool-retained API reaches that path today, although that tool cannot race its own initial harvest. R1 atomically seals and snapshots all fields named in the guarded table, disposes rejection/abandonment, and raises `ExtensionCapabilityError` at every later class-D `register_*`, `unregister_provider`, or direct/decorator `on` call boundary. It does not return inert `RegisteredFlag` or decorator values. Current activation has no timeout; the fix is current API correctness and also timeout-safe without adding timeout policy. `docs/extension-api.md`, changelog text, and tests for void, direct-handler, decorator-factory, and flag-registration return-shape families are mandatory. |
| C09 | Rejected activation keeps all previously live generation chrome; accepted removal clears it only after commit. | **Required in R2 and R4c; not narrowed.** Current pre-activation `clear_extension_chrome()` proves the gap. Existing `test_clear_extension_chrome_retires_generation_state_and_keeps_sticky_values` only characterizes clearing, not rejected-reload preservation. |
| C10 | Generation chrome writes target their owning sidecar; only live chrome is reconciled/rendered; rejected/retired sidecars close. | **Required in R2/R3/R4c.** `_LiveExtensionUiDriver` currently targets one session-global `ToolLoopTerminalUi._chrome`; the TUI paint lock prevents container tearing but cannot prevent a stale generation from updating the live surface. R2 stages, R3 carries the handle, and R4c publishes/reconciles it without nesting the TUI paint lock under the session mutex. |
...
| C12 | Imperative dialogs, editor text/paste, custom overlays, tools-expanded, and process theme changes are frozen chrome projections. | **Formally narrowed.** `_LiveExtensionUiDriver` executes these as immediate operation effects; they are not retained contribution maps and have no old/new projection to tear. TUI mutation/painting is serialized by `_paint_lock`; process theme is already explicitly non-transactional. R2/R4c still own all retained chrome/listener/autocomplete/editor-component/hidden-thinking-label projections. |
| C13 | User/custom messages live in generation-owned queue sidecars; append and live drain synchronize, only the live generation drains, and rejected/retired sidecars close. | **Required in R3/R4a, with terminal invocation in R5a.** Runtime outbox identity and live late binding are pinned by `test_generation_preserves_outbox_identity_and_ui_adapter_late_binding`; the cancellable extension-tool path makes the direct-list copy/clear lost update reachable as described above. R3 builds the sidecar. R4a converts every outbox writer—`_ActivationApi.send_user_message()`, `send_message()`/`sendMessage()`, and `commit_activation()`'s staged **user-message** flush—and `_CustomEntryRenderer.drain_extension_outboxes()`: under the session mutex each append performs closed-check+append atomically, each drain atomically detaches the live contents, and rejection/retirement close is serialized with both. Staged activation custom messages and `ExtensionCodingSessionControl` completion/entry/name/label/custom-message callables target accepted runtime/provider/tree/rendering/`CodingInputQueue` sinks, not these outboxes. On a closed queue, extension-facing activation send methods silently no-op and retain their existing `None` return; they raise no exception and emit no diagnostic. R5a's run-finally terminal transition invokes the already-built queue close as part of the same session-mutex terminal detach. |
...
| C19 | Frozen projections and successor state share no mutable contribution mapping/list. | **Required in R3.** Existing `ToolCapabilityState` and settings/keybinding states demonstrate immutable/copy-on-write values (`test_retained_capability_state_is_unchanged_by_later_publication`, `test_a_retained_settings_value_does_not_follow_a_later_reload`). R3 extends that proof to every retained extension projection; only explicit mutable sidecar handles remain mutable. |
| C20 | Active provider, thinking level, and active tools are generation-scoped mutable selection, rebased without losing a mutation accepted before cutover. | **Required in R3/R4c/R5b/R6.** `NativeToolCapabilities.publish()` already re-reads live active names under its lock; the full generation and provider/thinking owners are not yet generation-bound. R5b/R6 barriers pin accepted-before-gate survival. |
| C21 | Provider catalog contribution and coding provider binding publish coherently; refresh preserves history/usage, fallback replaces history/usage, and compaction survives. | **Required in R3/R4b/R4c and R6.** Current behavior is characterized by `test_reload_rebinds_active_extension_provider_factory`, `test_reload_falls_back_when_shadowing_extension_provider_is_removed`, and `test_rebind_clears_history_and_usage_but_preserves_compaction_suffix`, but publication still spans separate owners. |
...
| C23 | Active-tool capability state uses immutable candidate values and the session mutex. | **Landed foundation; required in R3/R4b/R5b for generation integration.** `NativeToolCapabilities.prepare_extensions()` / `publish()` and `tests/test_native_tool_capabilities.py` pin retained-state isolation and live-selection re-read. Context generation capture/refusal remains R5b. |
| C24 | Renderer selection remains pinned to the tool call that advertised it. | **Landed.** Captured and TUI cases are pinned by `test_captured_renderer_pins_result_renderer_to_its_call`, `test_captured_renderer_pins_result_renderer_when_tool_is_removed`, and the corresponding TUI tests. R3/R4b move the map into the coherent snapshot without changing that call pin. |
| C25 | Runtime plus parsed flags reject as one candidate. | **Landed.** `_reload_extension_generation()` parses before assigning `SessionExtensionGeneration`; `test_a_malformed_candidate_flag_retains_the_complete_prior_generation` proves old commands/flags remain together. R2 adds missing chrome preservation and R3 broadens the projection. |
...
append may check outside that section. Candidate-host staged messages are
snapshotted under the R1 host guard, that guard is released, and only then are
they appended/flushed under the session mutex; the two guards never nest. Sink
delivery and detached-value release happen after unlock. Every extension-facing
...
| --- | --- | --- | --- |
| R1 candidate `_ActivationApi`: `_staged` commands, `_staged_shortcuts`, `_hooks`, `_staged_tools`, `_staged_providers`, `_staged_unregistered`, `_staged_flags`, `_flag_values`, `_staged_message_renderers`, `_staged_entry_renderers`, `_failure`, `_staged_messages`, `_staged_custom_messages`, `_activated`, and one-way `sealed`/`disposed` state | `_execute_activation_entry()` failure decision and one atomic seal/freeze snapshot replacing `_collect_staged_contributions()`/`_activate_one()`/`_finalize_preloaded_extension()`'s separate reads; `get_flag()`/`RegisteredFlag.get_value()` through the guarded value view; duplicate/validity checks; send routing reads `_activated` only to choose/detach a target | every `register_*`, `unregister_provider`, direct/decorator `on`, staged send, first-failure recording, parsed flag-value application, successful activation/seal, and dispose | R1 candidate-host guard; all readers/writers take it; extension execution is unlocked; late class-D calls raise at this boundary; runtime send releases it before R4a's session section |
...
| Immutable `SessionExtensionGeneration` projection | Operation-local snapshot readers only; no second `current` read | Candidate builder before publication only | R3 build; R4a–R4c consumers |
| Generation queue sidecar: user/custom outbox contents and closed state, all guarded by the session mutex | R4a live drain through one operation snapshot; closed check in the same drain section | activation send methods and staged user-message flush perform closed-check+append atomically; R4a close on rejection/retirement; R5a invokes close on teardown | R3 value; R4a converts every append/drain/rejection/retirement writer; R5a terminal invocation. Detached delivery releases the session mutex before entering `CodingInputQueue`, forbidding `session mutex → mutation_io_lock` |
| Generation chrome sidecar: retained chrome/listener/autocomplete/editor-component values and closed state, guarded by that sidecar's sink-local guard | R4c immutable reconciliation snapshot after session unlock | retained class-B setters/registrations; R2 candidate close; R4c retired close; R5a terminal close | R2/R3/R4c; R5a terminal invocation; paint/disposal after unlock |
| `NativeToolCapabilities`: `ToolCapabilityState` pointer including `active_tool_names` | state and request/execution snapshots | candidate `publish()`; class-A `set_active_tools()` | Base locking landed; R3/R4b projection; R5b admission |
...
3. **Chrome:** after session unlock, take one chrome sink guard to close/snapshot,
   release it, then paint/dispose. Neither `session → chrome` nor
   `chrome → paint` exists.
...
| --- | --- | --- |
| R1 | `extension_runtime` activation/registration host: all command/shortcut/hook/tool/provider/unregistration/flag/value/renderer/message staged fields, `_failure`, `_activated`, and new sealed/disposed state; `extension_loader` activation adapter only for the host seam; reload composition wiring. Runtime sidecar append conversion, live chrome, selection, provider, and consumer dispatch are excluded. | One candidate-host guard taken by every listed reader/writer; one-way seal plus atomic freeze snapshot. It releases before any R4a queue flush/session section. |
| R2 | Candidate chrome/listener sink owner, reload orchestration, TUI chrome reconciliation adapter, and closed-sink no-op API documentation. No other projection or selection owner. | Candidate/chrome sink-local guard for closed-check+write/close; no session-mutex publication yet and no paint while held. |
| R3 | `session_generation` value, candidate queue/chrome sidecar builder, and composition adapters; no consumer conversion and no settings adapter because C16 selected omit. | Immutable candidate construction only; sidecars carry their named guard references, but append/drain/write conversion waits for R4a/R4c. |
| R4a | Command/shortcut/input/request/session-hook consumers and snapshot adapter; every generation-outbox writer (`_ActivationApi.send_user_message()`, `send_message()`/`sendMessage()`, `commit_activation()` staged user-message flush); `_CustomEntryRenderer.drain_extension_outboxes()` **only** (not rendering, accepted staged-custom-message delivery, or coding-session sends); queue rejection/retirement close; provider-header request-local trust capture; reload composition only for those close/deletion duties; closed-queue API documentation. | Candidate host guard only to detach staged messages, then release; existing session mutex as the queue guard for closed-check+append, atomic detach/drain, close, and one operation snapshot. No nested edge. |
...
| --- | --- |
| Retained frozen projection; successful coherent commands/hooks/tools/providers/renderers/flags/shortcuts/UI; derived projection failure | **Required in R3/R4a–R4c**, closed in R7 integration. |
...
| Rejected cleanup cannot leave the gate open | Gate `finally` **landed**; R1 must prove cleanup occurs after gate closure and cannot re-open registration. |
| Activation API exposes no class-A port | **Landed** by `PipyExtensionAPI`/`_ActivationApi` shape; R1 retains it. |
| Candidate user/custom outboxes are not delivered on rejection | Activation staging **landed** for synchronous rejection; R1/R3/R4a add sealed/closed abandonment and synchronized live delivery. |
...
| Rejected listeners/chrome requests; post-seal contribution; normal published handler queue/chrome use | **Required in R1–R2/R4a/R4c**. |
| Candidate flag failure keeps runtime/flags | **Landed**; R2 adds old chrome and R3/R4c add every projection. |
...
| Removed/disabled chrome clears only after successful commit | **Required in R2/R4c**. |
