...
Status: R0-reconciled completion contract; R1–R3c3 are shipped and R3 is
complete; R4a is active/next. The original
...
the R3a/R3b values and invokes the R3c1a–R3c1c owner APIs. Production still lacks
broader R4 operation-snapshot consumer adoption, generation-bound stale mutation
refusal, and atomic `set_model` admission; R4a–R6 retain those owners.
The
...
validation and publication. R3c2 routing and R3c3 composition are shipped; active/next R4a adopts operation snapshots and live queue synchronization.
...
| C04 | A generation has one immutable contribution/configuration projection. | **Landed for installed startup/reload generations in R3c3; R4a–R4c retain consumer adoption.** Both production paths call the same R3a builder, and `SessionExtensionGeneration` now carries that exact frozen `ExtensionProjection` with its runtime, flags, routing owner, and chrome token through the single `SessionGenerationRef` pointer. Reload publishes the pointer and temporary legacy owner assignments in one accepted mutex section; no second projection pointer exists. Legacy consumers still read their established adapters until the bounded R4 moves delete them. |
| C05 | Operations take one generation snapshot at start and keep it for their duration. | **Landed in R3c3 for the custom-render drain operation only; broader adoption remains R4a–R4c.** The sole production `_CustomEntryRenderer` construction now receives `SessionGenerationRef.snapshot`; each drain takes that one coherent snapshot and resolves its routing owner from the captured generation. Direct custom tree/render/input delivery deliberately remains R1-compatible and does not consult routing. Other `_RunControlState.extension_generation` consumers still read live state per access and remain pending. |
...
| Installed immutable `ExtensionProjection` in `SessionExtensionGeneration` | R3c3 renderer-drain snapshot plus R4a–R4c pending consumers | R3a pure builder; R3c3 startup/reload installation | R3a construction and R3c3 installation landed; R4a–R4c retain consumer migration |
...
it is not routing install authority. The shipped R3c3 production set contains exactly two
...
two `release_pending_route` callback references, one for each path; three
`retire_route()` calls for superseded-route, reload-refusal, and startup-
ownership-refusal cleanup; two underlying `GenerationMessageRouting.retire()`
calls (the projection forwarding method and the legacy no-projection branch);
and the one combined `SessionGenerationRef.accept_prepared_reload()` retirement
and owner-publication call. The startup install is not a separate helper. It also
contains exactly one `_CustomEntryRenderer` construction wired to
