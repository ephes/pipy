...
| Provider access | Broad provider/model registry through Pi's AI package, including subscription and API-key paths. | Twelve real provider adapters behind `ProviderPort` plus the fake bootstrap, now fronted by a pipy-owned catalog (`native/catalog.py`/`catalog_data.py`) with multiple rows per provider, a `models.json` custom-provider/override loader (`native/models_json.py`), the layered matcher (`native/model_resolver.py`), per-request auth resolution + availability gate (`native/auth_store.py`), the stdlib OAuth registry for Anthropic/Copilot/Codex (`native/oauth_providers.py`), thinking levels (`native/thinking.py`), routing (`native/routing.py`), `--list-models [search]`, and a full-catalog `/model` selector (`native/catalog_state.py`). ds4 is reframed as a `models.json` custom provider (`native/ds4.py`). Gated by `scripts/parity_checks/provider_catalog_conformance.py` (18/18); design in `docs/provider-catalog.md`. Remaining product follow-ons: `/scoped-models` and `--models` Ctrl+P live cycling, plus live Anthropic/Copilot login UX beyond the tested OAuth core. |
