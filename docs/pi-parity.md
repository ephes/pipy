...
| Startup orientation | Implemented Pi-parity pass | The shell prints Pi-equivalent compact startup chrome on stderr: title, one-line dim controls strip, `Type /help …` affordance, loaded-only `[Context]`/`[Skills]`/`[Prompts]`/`[Extensions]` sections, with TTY-only ANSI styling and plain captured-stream fallback. |
| Active prompt state | Implemented | The prompt is a simple `> ` leader framed by horizontal separator lines; a dim two-line footer below the input area shows the workspace cwd and a `<workspace> · <provider>/<model> · turns N/M · read R/L` summary (plus `· proposal ready` / `· verify ready` when applicable). |
| Terminal input runtime | Pi-parity Tab discovery | The input adapter preserves plain captured-stream fallback, can use optional prompt-toolkit line-editor input with slash-command completion (descriptions through `display_meta`), explicit file/path completion, completion-only `@file` reference labels, and multiline entry on real TTY streams. When prompt-toolkit is unavailable, a stdlib `readline` adapter (`--input-runtime readline`) takes over so Tab surfaces the slash-command menu in the live REPL without declaring a runtime dependency. Richer editor behavior (history, resize-resilient TUI) remains deferred. |
