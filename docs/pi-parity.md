...
44/50 with 7 big features green. D4 (skills loading), D5 (prompt templates),
and D6 (custom slash commands) are now green: the discovery helpers were
...
Red rows are B7 (`bash`), D7 (themes), D8 (image attachments), E2 (session
compaction), and E3 (session branching). Each remaining red row corresponds
to a file-existence-checked surface the 2026-05-26 audit identified as dead
code with no production consumer; rewriting those checks to verify real
behavior is justified only once a runtime consumer exists for each. The
...
- Full interactive terminal UI parity beyond the current tool-loop TTY shell:
  ...
  in the product TUI.)
- Pasted images and broader keyboard shortcut handling. (User-directed
  `@file` content reads now ship: a genuine prompt naming workspace files with
  `@path` loads bounded excerpts into the next provider request in both REPL
  modes and the product TUI.)
...
- Richer resource loading beyond AGENTS/CLAUDE-style instruction discovery.
  ...
  wired into both REPL product paths. The `themes` discovery helper was
  removed in the 2026-05-26 cleanup and remains deferred until a chrome
  consumer exists.
...
| Extension system | First-class extensions, skills, prompt templates, themes, custom commands, and UI hooks. | Skills, prompt templates, and custom slash commands now load at runtime from pipy-owned `.pipy/skills`, `.pipy/templates`, and `.pipy/commands` stores (workspace + global) through `pipy_harness.native.resources`, wired into both REPL product paths; the `[Skills]` chrome section lists the loadable skill names a user can run with `/skill`. A general extension/package loader, themes, and UI hooks remain deferred until a consumer exists. This is deliberately **not** a general extension API — only the three bounded resource kinds load, through the existing provider/session/tool/archive boundaries. |
