...
- Full interactive terminal UI parity beyond the current tool-loop TTY shell:
  richer selectors/overlays and persistent cross-session history. (Prompt
  history recall, bracketed paste, undo/redo, and resize handling now ship in
  the product TUI.)
- Automatic file-content reads from `@file` references, pasted images,
  persistent cross-session history, and broader keyboard shortcut handling.
...
| UI | Rich TUI with editor, footer, selectors, overlays, and extension UI. | Product tool-loop sessions use a pipy-owned inline-scrollback TUI (committed history in native scrollback, live input/footer frame pinned at the bottom, full window height) with prompt/footer ownership, slash menu, submitted prompt bands, tool rows, active assistant output, active-Escape abort rendering, a read-only `/settings` overlay, an interactive `/model` provider/model selector, executable `/login`/`/logout`, and daily-driver editor ergonomics (prompt history, bracketed paste, undo/redo, resize handling). The no-tool REPL remains line-oriented with compact startup chrome, grouped help, `/status`, `/settings`, a state-aware prompt label, and an optional prompt-toolkit input adapter with command/path completion, completion-only `@file` reference labels, and multiline entry. Broader selectors/overlays, persistent cross-session history, and extension UI remain deferred. |
