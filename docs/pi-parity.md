...
decision/read APIs and request-scoped extension `before_provider_headers`
mutation now ship. The remaining `agent_settled` follow-on (the
extension-surface hook), durable extension entry renderers,
...
| Extension system | First-class extensions, skills, prompt templates, themes, custom commands, and UI hooks. | **Partial, Pi-shaped but not Pi-equivalent.** Runtime skills, prompt templates, custom slash commands, and chrome themes ship. A Python-only extension runtime now covers local discovery/activation, extension slash commands and shortcuts, lifecycle/input/before-agent-start hooks, `tool_call` permission gates, extension tools, `tool_result` transforms, minimal `ctx.ui.notify`, simple `ctx.ui` select/input/confirm/editor/status/working primitives plus editor text helpers, a golden conformance extension, provider registration wired through the native catalog/model selector, local-path and managed-git package runtime composition, package `update`, live-session hooks/controls (`user_bash`, `before_provider_request`, `before_provider_headers`, session-operation gates, active tool/model/thinking controls), dynamic extension flags for tool-loop `pipy repl` via `ExtensionFlag` + `ctx.flags`, custom session-entry/message rendering via `api.register_message_renderer` + `ctx.append_entry`, render-once custom tool renderers, rich message renderers, live custom editor component integration, footer data with branch-change callbacks, idle custom-message delivery, and extension OAuth-provider `/login` wiring. The package CLI (`pipy install/remove/uninstall [-l]`, `list`, `config`, `update`) records sources and filters; installed package resources flow through discovery for extensions/skills/prompts/themes at lowest precedence and honor `+/-pattern` filters. Per-run `--extension`/`--skill`/`--prompt-template`/`--theme` source paths and matching `--no-*` discovery cutoffs also ship. Compared with Pi, remaining gaps include TypeScript source compatibility (intentionally non-goal), the extension-surface `agent_settled` hook, durable entry renderers, broader custom editor/component-library parity, richer multi-widget extension UI, live tool-render invalidation beyond the render-once snapshot, broader dynamic-flag integration, PyPI/npm package sources, and the RPC extension-UI channel. See [extension-api.md](extension-api.md). |
