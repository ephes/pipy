...
Red rows are B7 (`bash`), D7 (themes), D8 (image attachments), and E5
(dynamic provider swap — implemented in both REPL modes via
`NativeReplProviderState.select_model`, but its parity-score row still greps
for a removed `dynamic_provider` helper). The boundaries below remain Pi-class
surfaces that pipy has not yet closed, several because the previously dormant
helper was removed for being unwired rather than because it was ever live:
...
- The dormant `image_attachment` helper was removed in the 2026-05-26
  cleanup; its parity row (D8) remains `Deferred`. (`session_compaction` and
  `session_branching` were also removed then, but both have since been
  reimplemented and wired into the live product paths — see the rows above.)
