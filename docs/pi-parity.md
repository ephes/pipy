...
| Active prompt state | Implemented | The no-tool REPL keeps the separator-framed prompt and persistent two-row bottom status block. In real TTY tool-loop sessions, `pipy_harness.native.tui.ToolLoopTerminalUi` owns an alternate-screen frame so prompt editing, submitted messages, loader frames, assistant text, and the footer repaint without stale scrollback rows. The latest submitted user block is preserved when long tool output overflows the history region, while the newest tool/assistant rows remain visible above the pinned input/footer. The status line keeps the same `$cost (plan) used%/budget (suffix) … (provider) model • effort` shape with token arrows when usage is available. TUI verification now includes a stdlib ANSI screen-cell model plus tmux product-path artifacts that locate visible strings, cursor position, inferred input/footer rows, and key cell attributes; `scripts/tmux_pi_comparison_verify.sh` also captures Pi and writes row/column plus cell-attribute delta artifacts for prompt, output, footer/status, input, live cursor, and drawn cursor positions. |
