...
| Startup orientation | Implemented Pi-parity pass | The shell prints Pi-equivalent compact startup chrome on stderr: title in sage truecolor (with 16-color fallback), one-line dim controls strip referencing `/ commands`, `Type / to open the command menu` affordance, loaded-only `[Context]`/`[Skills]`/`[Prompts]`/`[Extensions]` sections in yellow, purple input separator, TTY-only ANSI styling, and plain captured-stream fallback. Chrome rendering is shared between the no-tool and bounded tool-loop sessions through `pipy_harness.native.chrome`. |
| Active prompt state | Implemented | The prompt is a simple `> ` leader framed by horizontal separator lines; a dim two-line footer below the input area shows the workspace cwd and a `<workspace> · (provider) model · turns N/M · read R/L` summary (plus `· proposal ready` / `· verify ready` when applicable). The default `pipy` invocation auto-picks an available real provider (openai-codex OAuth or keyed providers in priority order), so the footer surfaces `(provider) model` rather than `fake/fake-native-bootstrap` whenever credentials are configured. |
