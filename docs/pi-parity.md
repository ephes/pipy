...
| Terminal input runtime | Narrow first slice | A small input adapter preserves plain captured-stream fallback and can use optional prompt-toolkit line-editor input with slash-command and explicit file/path completion on real TTY streams. Richer editor behavior remains deferred. |
...
- Full interactive terminal UI with editor, persistent footer,
  ...
  and explicit file/path completion boundaries.
- File references with `@`, multiline editing, pasted images, and keyboard
  shortcut handling.
...
| UI | Rich TUI with editor, footer, selectors, overlays, and extension UI. | Line-oriented REPL with compact startup chrome, grouped help, `/status`, a state-aware prompt label, and an optional prompt-toolkit input adapter with command/path completion; richer editor behavior remains deferred. |
