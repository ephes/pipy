...
| SDK / RPC embedding | SDK module implemented | `pipy_harness.sdk` exposes `make_native_run_request(...)`, `run_native(...)`, and the public value objects (`RunRequest`, `RunResult`, `HarnessStatus`, `CapturePolicy`, `HarnessRunner`, `ProviderPort`, `StreamChunkSink`) for in-process headless Python embedding; see [sdk.md](sdk.md). Pi-style stdin/stdout JSON/RPC automation is specified in [automation-rpc.md](automation-rpc.md), while network/socket daemons remain deferred. |
