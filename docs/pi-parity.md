...
- Extension/package parity beyond the landed Python core: installed local-path
  package runtime composition now ships for extensions, skills, prompts, and
  themes at lowest precedence with `+/-pattern` filters, per-run source-loading
  flags now load explicit extension/skill/prompt-template/theme paths while
  matching `--no-*` flags disable default discovery, and live-session hooks now
  cover `user_bash`, `before_provider_request`, session/tree/compaction gates,
  and dynamic active tool/model/thinking controls. Pi-level rich TUI extension
  UI, custom message/tool rendering, dynamic extension flags, extension
  state/session-manager helpers, remote packages, OAuth-provider extension
  registration, and `update` remain future work.
...
| Extension system | First-class extensions, skills, prompt templates, themes, custom commands, and UI hooks. | **Partial, Pi-shaped but not Pi-equivalent.** Runtime skills, prompt templates, custom slash commands, and chrome themes ship. A Python-only extension runtime now covers local discovery/activation, extension slash commands and shortcuts, lifecycle/input/before-agent-start hooks, `tool_call` permission gates, extension tools, `tool_result` transforms, minimal `ctx.ui.notify`, a golden conformance extension, provider registration wired through the native catalog/model selector, local-path package runtime composition, and live-session hooks/controls (`user_bash`, `before_provider_request`, session-operation gates, active tool/model/thinking controls). The local-path package CLI (`pipy install/remove/uninstall [-l]`, `list`, `config`) records sources and filters; installed local-path package resources flow through discovery for extensions/skills/prompts/themes at lowest precedence and honor `+/-pattern` filters. Per-run `--extension`/`--skill`/`--prompt-template`/`--theme` source paths and matching `--no-*` discovery cutoffs also ship. Compared with Pi, remaining gaps include TypeScript source compatibility (intentionally non-goal), rich TUI extension UI and custom rendering, dynamic extension flags/message renderers, extension state/session-manager helpers, remote npm/git/PyPI-style packages, OAuth-provider extension registration, and `update`. See [extension-api.md](extension-api.md). |
