...
| Active prompt state | Implemented | The no-tool REPL keeps the separator-framed prompt and persistent two-row bottom status block. In real TTY tool-loop sessions, `pipy_harness.native.tui.ToolLoopTerminalUi` owns an alternate-screen frame so prompt editing, submitted messages, loader frames, assistant text, streamed reasoning, and the footer repaint without stale scrollback rows. The latest submitted user block is preserved below retained startup/context rows when terminal height or long tool output overflows the history region, while the newest tool/assistant rows remain visible above the pinned input/footer. The submitted-prompt shaded band matches Pi's three-row shape: spacer, prompt, spacer. Model-selected tool commands render as Pi-style `$ ...` command rows with bounded result previews. The status line keeps the same `$cost (plan) used%/budget (suffix) … (provider) model • effort` shape with token arrows when usage is available. TUI verification now includes a stdlib ANSI screen-cell model plus tmux product-path artifacts that locate visible strings, including wrapped prompt/output text, cursor position, inferred input/footer rows, key cell attributes, final prompt background rows, and the final viewport text. `scripts/tmux_pi_comparison_verify.sh` captures Pi and writes row/column, cell-attribute, prompt-background, and normalized final-viewport delta artifacts so ordinary prompts can expose visible answer or retained-chrome drift even when a selected expected-output marker appears on both sides; the required gate covers default 100x30 and short 100x24 panes. |
...
| Tool call / output rendering | Pi-shape inline blocks + TUI frame | Real TTY tool-loop sessions render tool calls/results into the TUI history region through `ToolLoopTerminalUi`, while captured streams still use `_ToolLoopRenderer`'s readable stderr blocks. Errors are tagged, long result bodies are bounded, ANSI styling is gated on TTY detection and `NO_COLOR`, and captured logs stay readable. |
...
- Full interactive terminal UI parity beyond the first tool-loop TTY shell:
  richer editor behavior, slash-menu overlays inside the TUI frame,
  model/status controls, selectors, persistent history, and resize handling.
