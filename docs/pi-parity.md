...
| Terminal input runtime | Planned next slice | A terminal-layer checkpoint selected a narrow `prompt-toolkit` line-editor adapter investigation while keeping the current plain line-oriented REPL as the fallback. |
...
- Full interactive terminal UI with editor, persistent footer,
  ...
  selected narrow `prompt-toolkit` line-editor adapter investigation.
...
| UI | Rich TUI with editor, footer, selectors, overlays, and extension UI. | Line-oriented REPL with compact startup chrome, grouped help, `/status`, and a state-aware prompt label; a narrow `prompt-toolkit` line-editor adapter investigation is the next planned UI boundary. |
