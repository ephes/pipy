...
| --- | --- | --- |
| Local-first terminal coding agent | Partial | `pipy` and `pipy repl` start a native shell in the current workspace. The bounded tool-loop REPL now enters a pipy-owned alternate-screen TTY shell: submitted user messages, active assistant output, and transient working state flow through the history area while input and footer/status stay in a stable frame. The no-tool REPL and captured-stream fallback remain line-oriented. |
...
| `/login`, `/logout`, `/model`, `/settings` | Implemented narrow shell commands | Commands are local, late-bind provider selection or inspect safe local state, and do not create provider turns or archive auth material. `/settings` prints provider/model labels and availability reasons only. The no-tool `/settings` text and the product-TUI `/settings` overlay share one read-only builder (`pipy_harness.native.repl_state.settings_overlay_lines`), so both surfaces show the same active selection, registered defaults, and per-provider local availability reasons; `/model`, `/login`, and `/logout` remain no-tool REPL commands only. |
...
| Active prompt state | Implemented | The no-tool REPL keeps the separator-framed prompt and persistent two-row bottom status block. In real TTY tool-loop sessions, `pipy_harness.native.tui.ToolLoopTerminalUi` owns an alternate-screen frame so prompt editing, slash-menu overlays, submitted messages, loader frames, assistant text, streamed reasoning, and the footer repaint without stale scrollback rows. Streamed reasoning rows render in italic dim via `ChromeStyle.dim_italic`, matching Pi's italic thinking voice and pipy's captured-stream fallback renderer. During active provider turns, the main thread keeps a raw-mode Escape watcher alive while the provider runs in a worker; Escape clears the loader, suppresses late stream chunks, returns to the prompt, and renders Pi-style red `Operation aborted`. The latest submitted user block is preserved below retained startup/context rows when terminal height or long tool output overflows the history region, while the newest tool/assistant rows remain visible above the pinned input/footer. The submitted-prompt shaded band matches Pi's three-row shape: spacer, prompt, spacer. Model-selected tool commands and result previews render as Pi-style shaded rows. Typing `/settings` opens a read-only settings/status overlay (`ToolLoopTerminalUi.show_settings`) rendered as a settings history block through the same whole-frame paint path; it shows the same safe provider/model/status information and availability reasons as the no-tool `/settings` command and creates no provider turn. The status line keeps the same `$cost (plan) used%/budget (suffix) … (provider) model • effort` shape with token arrows when usage is available. TUI verification now includes a stdlib ANSI screen-cell model plus tmux product-path artifacts that locate visible strings, including wrapped prompt/output text, cursor position, inferred input/footer rows, key cell attributes, final prompt background rows, named visual-region rows, and the final viewport text. `scripts/tmux_pi_comparison_verify.sh` captures Pi and writes row/column, cell-attribute, prompt-background, visual-region, and normalized final-viewport delta artifacts so ordinary prompts can expose visible answer, styling, or retained-chrome drift even when a selected expected-output marker appears on both sides; the required gate covers default 100x30 and short 100x24 panes. |
| Terminal input runtime | Pi-parity `/` menu + Tab discovery | The default `auto` runtime selects the product TUI on real TTY tool-loop sessions and a stdlib-only `slash-menu` raw-mode line editor for the no-tool line-editor path. In the product TUI, typing `/` opens a Pi-like command menu beneath the input separator with slashless command labels, dim descriptions, an accent-colored `→` selection row, Up/Down navigation, Enter to submit the selected command, Tab to accept without submitting, and Esc to close without exiting; it lists only local commands the tool-loop dispatcher can execute (`help`, `settings`, `exit`, `quit`) so the menu does not advertise inert commands. `settings` opens the read-only settings/status overlay described under Active prompt state. The broader stdlib/prompt-toolkit line-editor paths keep the full local slash-command completion set. `scripts/tmux_tui_input_verify.sh` now exercises Escape and slash-menu frames through the real product command without submitting a provider turn. The next fall-throughs are optional prompt-toolkit input with slash-command completion (descriptions through `display_meta`), explicit file/path completion, completion-only `@file` reference labels, and multiline entry; then the stdlib `readline` adapter for Tab discovery without a runtime dependency; then plain stdin/stderr for captured streams. Richer editor behavior (history, resize-resilient TUI, mouse selection) remains deferred. |
...
- Live runtime wiring for streaming in `--repl-mode no-tool` and
  ...
  `Helper implemented`.
- Full interactive terminal UI parity beyond the first tool-loop TTY shell:
  richer editor behavior, model/status controls, selectors, persistent history,
  and resize handling.
...
| UI | Rich TUI with editor, footer, selectors, overlays, and extension UI. | Product tool-loop sessions use a pipy-owned alternate-screen TUI with prompt/footer ownership, slash menu, submitted prompt bands, tool rows, active assistant output, and active-Escape abort rendering. The no-tool REPL remains line-oriented with compact startup chrome, grouped help, `/status`, `/settings`, a state-aware prompt label, and an optional prompt-toolkit input adapter with command/path completion, completion-only `@file` reference labels, and multiline entry. Richer editor behavior, selectors, overlays, and extension UI remain deferred. |
...
| Extension system | First-class extensions, skills, prompt templates, themes, custom commands, and UI hooks. | No runtime extension/package loader today. `[Skills]` and `[Extensions]` chrome sections are display-only directory labels; skills, prompt templates, custom slash commands, themes, runtime slash commands, extensions, and package loading remain deferred until a consumer exists. |
