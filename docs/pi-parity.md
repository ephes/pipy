...
- Extension/package parity beyond the landed Python core: installed local-path
  ...
  `user_bash`, `before_provider_request`, session/tree/compaction gates, and
  dynamic active tool/model/thinking controls. Pi-level rich TUI extension UI,
  custom message/tool rendering, broader dynamic extension flag integration, extension
  state/session-manager helpers, PyPI/npm package sources, and OAuth-provider
  extension registration remain future work.
