...
2. Extract `pipy_harness.native.providers._chat_completions_shared` for
   ...
   `02:F3`. **Relocation in progress (migration Slice 5.2-chat, cuts 1–2):**
   ...
   `pipy_harness.native.providers.ds4`) in cut 1, and the Mistral and OpenRouter
   clones moved verbatim to `pipy_harness.native.providers.mistral` and
   `pipy_harness.native.providers.openrouter` in cut 2 — all translation-only
   ...
   its `reasoning_effort` passthrough, and OpenRouter keeps its nested
   `reasoning` normalization. Remaining: relocate Cloudflare (cut 3), then the
   `_chat_completions_shared`/wire consolidation and helper de-duplication
   (cut 4).
