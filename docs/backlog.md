...
- Interactive input ergonomics: the input-adapter boundary preserves plain
  ...
  command menu beneath the input with names, descriptions, reverse-video
  selection highlight, Up/Down navigation, Enter to submit the selection,
  Tab to accept without submitting, and Esc to close), still supports
  optional prompt-toolkit line-editor input when the package is installed
  or explicitly selected, and falls back to a stdlib `readline` adapter
  (`--input-runtime readline`) for Tab-driven discovery without a runtime
  dependency. Empty-input Tab surfaces the full slash-command menu in the
  readline path; partial `/<prefix>` Tab filters it. Slash-command
  completions now carry descriptions through prompt-toolkit's `display_meta`
  and through `readline.set_completion_display_matches_hook` where the
  underlying backend honors it (libedit on macOS gracefully degrades to the
  default columnar match listing). Workspace-relative path completion for
  explicit file commands, completion-only `@file` references in ordinary
  prompts and supported command free-text, multiline entry in the
  prompt-toolkit path, the cursor-position warning hardening, and the
  LF-encoded newline key sequences from earlier slices are preserved.
  Explicit file-context commands continue to share a two-successful-excerpt
  REPL budget. Resilient terminal resize behavior, persistent history, and
  a fuller TUI remain future slices.
...
- Streaming Output Parity Track (2026-05-26): closed parity-criterion row
  C14, raising the score from 44/50 to 45/50 (90%) with 8 "big" features
  green. Added a `StreamChunkSink` callable boundary on `ProviderPort`,
  wired `OpenAICodexResponsesProvider` to push parsed
  `response.output_text.delta` events through the sink, and exposed
  `pipy run --stream` for text/json output modes. Other tool-capable
  providers (`openai`, `openrouter`, plus the eight non-tool real
  adapters) accept the keyword and stay on their buffered paths. The
  metadata-first archive contracts, the opt-in `--archive-transcript`
  sidecar, `.git` default-deny, no-new-runtime-deps, and the existing
  slash commands are unchanged. Score check: `just parity-score`.
...
- Native grep tool (slice 7 of the Tool-Loop Parity Track):
  `pipy_harness.native.tools.grep.GrepTool` searches for a literal
  (non-regex) string across workspace files with bounded output. When
  `rg` is on `PATH`, the tool invokes
  `subprocess.run(["rg", "--no-heading", "--line-number", "--color=never",
  ...
  `max_output_bytes` (default 32 KiB, capped at 256 KiB), and append the
  stable `"... (truncated)"` marker on overflow; no matches yields
  `"(no matches)"`. The tool reuses `_validate_workspace_relative_path`,
  `_is_ignored_or_generated`, and `_is_relative_to` so `.git` and
  ignored paths are refused identically; absolute paths and parent
  traversal raise `ToolArgumentError`. `production_tool_registry()` now
  returns `{"read": ..., "ls": ..., "grep": ...}`. Tests pin: definition
  and schema; literal matches across multiple files; refusal of `.git`
...
- OpenAI Responses + OpenAI Codex Tool-Call Parity Track slice 3
  (`OpenAICodexResponsesProvider` tool-call wiring):
  `OpenAICodexResponsesProvider` now advertises
  `supports_tool_calls=True` and drives the existing bounded tool
  loop end-to-end against the Codex Responses streaming endpoint.
  The body builder serializes the loop's `messages` envelope into
  the Codex streaming `input` list (user/assistant text plus
  `function_call` items), serializes `ToolResultMessage` as
  `function_call_output` items, and declares tools via the flat
  Responses-API tool shape from `available_tools`. The SSE
  assembler accumulates function-call items across
  `response.output_item.added`,
  `response.function_call_arguments.delta`,
  `response.function_call_arguments.done`, and
  `response.output_item.done` events keyed by `item_id`, prefers
  the explicit `done` arguments over accumulated deltas, surfaces
  the calls in arrival order on `ProviderResult.tool_calls` keyed
  by `call_id`, and allows missing final text whenever tool calls
  are present. Legacy single-turn callers leave `messages` empty
  and `available_tools` empty; the body builder then keeps the
  previous behavior with no `tools` field and emits no
  `tool_calls`. New tests in
  `tests/test_native_openai_codex_tool_calls.py` pin
  serialization, SSE-delta assembly,
  `response.output_item.done`-only finalization, multi-call
  ordering, tool-result envelope shape, and the legacy single-turn
  path; `tests/test_tool_loop_end_to_end_openai_codex.py` closes
  the full loop against a stubbed SSE transport. The
  `test_real_providers_advertise_tool_call_support_correctly`
  assertion now confirms all three real adapters
