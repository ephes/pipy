...
at `6e38076`; D7 direct-bash cancellation and incremental output are complete
at `66189ed`.

The scheduled grooming after D6a1/D7a1/D7b1 inspected summary-safe session
search/list results, finalized D6a/D7 summaries, the actual public facade/tree/
transition owners, and remaining D6c/D8 contracts at `66189ed`. It found no
prior D6b or D8 decision record that changes the queue. D6a's canonical-path
lease and strict reopen are complete, and D7 now has exact cancellable operations
...
The scheduled grooming after D6b1 `cd92191`, D6b2 `6b06613` and D6b3
`b370a8a` inspected summary-safe session search/list results and the current SDK,
CLI, terminal-command, product-facade, coordinator, wiring and lifecycle owners.
The targeted archive search found no prior D6c decision that changes the queue.
Two read-only Terra inventories found that `run_native` and
`make_native_run_request` have only documentation and test callers, while the
installed `pipy run --agent pipy-native` path still uses
`PipyNativeAdapter`/`NativeHarnessCompatibilityRuntime` for its distinct
metadata archive and `RunResult` contract. Interactive, JSON, print and RPC
already use the canonical coding/agent product loop; the supported Python
product facade exposes the same product owner but lacks a checked-in executable
example.

The terminal session-command frontend still mutates trees outside the D6b
coordinator: `/new` creates directly, `/resume` permissively opens and replaces,
and `/fork`/`/clone` reopen the durable source rather than snapshotting the
guarded active tree. That lifetime holds no canonical lease slot. Adoption is
therefore not a mechanical callback substitution. D6c1 first makes the already
supported product embedding concrete without changing runtime behavior. D6c2
then fixes the narrow `/resume` selection contract before D6c3 code. Picker
cancel/current selection and terminal path/id resolution remain presentation;
...
D6c2 fixes the terminal `/resume` adoption contract in `session-tree.md`,
`harness-spec.md` and `architecture.md` against read-only source/test evidence at
`b952ad2`. The stream-driven terminal lifetime will claim its initial persistent
tree before command execution, retain one canonical lease slot through every
successful switch, and finish that slot through existing controller teardown.
Resolved direct and picker targets keep presentation-owned lookup but delegate
canonical equality, candidate claim, strict load, workspace validation, guarded
tree publication and one history/extension-input rebuild to the existing D6b
coordinator. Same-path selection is a no-op. Expected prepublication target
failures retain the old tree and lease and become bounded terminal diagnostics;
unexpected publication errors and failures after publication remain fatal.
Listing, rename/delete, picker mechanics, `/tree`, import/export, `/new`,
fork/clone and compatibility SDK retirement remain excluded.

Focused architecture/document validation passed 255 tests. The full gate passed
6,154 tests with two skipped, plus Ruff, formatting, Mypy, docs-build, diff
hygiene and unchanged interpreter links. The first focused Terra planning review
returned CLEAN after inspecting the complete diff and the coordinator,
command/composition, controller, guarded-tree and focused acceptance owners. The
two-round planning cap closes without a second round.

D6c3 may write `src/pipy_harness/native/coding/session.py`,
...
`src/pipy_harness/native/repl/session_commands.py` and, only if the reviewed
terminal call shape requires it, the existing
`src/pipy_harness/native/repl/session_transition.py`. Focused tests are
`tests/test_native_coding_session_tree.py`,
`tests/test_native_coding_session_lifetime.py`,
`tests/test_native_coding_session_controller.py` and structural inventories
...
D6c4 re-inventories the remaining replacement and compatibility entry points at
`a25f67d`. Summary-safe session search found no prior D6c decision that changes
the queue. `run_native` and `make_native_run_request` still have only docs and
test callers, but `pipy run --agent pipy-native` remains a production caller of
`PipyNativeAdapter`/`NativeHarnessCompatibilityRuntime` and its distinct archive
and `RunResult` contract. Compatibility retirement is therefore not the next
mechanical deletion. Terminal `/fork` and `/clone` still reopen the durable
source, while `/import` owns confirmation, workspace fallback and import error
presentation; each needs its own contract.

Terminal `/new` is selected first. It has no reference, picker or load policy,
and the D6b coordinator already owns persistent fresh-tree creation and the
state-first lease handoff. More urgently, D6c3's terminal lifetime now owns a
canonical lease slot, while the old `/new` path replaces the guarded tree
without updating that slot. D6c5 closes this ownership gap without combining
the other replacement commands. It adds a terminal-only coordinator operation:
persistent sources reuse the existing public create/claim/publish/handoff path;
ephemeral sources preserve the current Pi-aligned in-memory `/new` behavior and
require an empty slot. The public/RPC refusal for ephemeral sources remains
unchanged.

The terminal continues to own command classification, the detailed
`session_before_switch` diagnostic, sanitized success output and the standard
footer. Success orders gate, create, candidate claim for persistent trees,
guarded tree publication, slot handoff and old-claim release, one history
rebuild, one extension-input clear, success diagnostic and footer. Ephemeral
success omits claim/handoff and leaves the slot empty. `/new` retains its lack
of custom-entry redraw. A veto occurs before creation; persistent candidate
lease conflict is recoverable, retains the old usable tree/lease/history/input,
and may leave the already-created durable artifact. Create or unexpected
pointer-publication failure remains fatal. Rebuild or clear failure after
publication remains state-first and fatal; controller teardown releases the
...
### D6b — Public and RPC session transitions

At `66189ed`, the public `ProductSession` supports create, exact-path reopen,
submit, cross-thread cancel, idle snapshot and idempotent close. It has no fork,
...
D6b1 implements the neutral canonical lease registry/slot and the one native
transition coordinator, then exposes public `fork()` and `clone()` through the
existing prepared lifetime. It snapshots the exact guarded in-memory branch,
runs the existing fork gate without holding queue/tree locks, creates fresh
mapped lineage in the source directory, publishes the tree and lease handoff,
and rebuilds once. Public and future admitted-control entry points remain
separate; D6b1 adds no RPC or terminal handler.

Root's focused gate passed 98 public-product, fork/clone, extension-lifecycle and
session-lifetime tests. They cover real allow/veto/crash hooks, exact operation
vocabulary, arbitrary exact entries, reference remapping, D6a lease regression,
prepublication refusal, postpublication fail-closed teardown, thread/reentry and
immutable results.
The first independent Terra code review reran all 98 focused tests and returned
CLEAN; no repair or follow-up round was warranted. Root's full gate then exposed
two stale exhaustive contract-test inventories for the intentional SDK exports
and candidate-abort call. Updating those inventories was test-only; the 98
focused tests still pass. The repaired full gate passed 6,122 tests with two
skipped, plus Ruff, formatting, Mypy, docs-build and diff checks; the virtualenv
interpreter links and configuration stayed unchanged.

D6b2 extends that coordinator and prepared public lifetime with
`new_session()` and exact-path `switch_session()`. Fresh replacement gates before
creating an empty sibling tree. Existing replacement canonicalizes first,
short-circuits the same-path no-op, then gates before claiming and strict-loading
the candidate. Both publish through the guarded lease handoff and rebuild the
existing history once; RPC adoption remains D6b3. Root independently passed 107
public product, session-lifetime, extension-lifecycle and fork/clone regression
tests initially. Terra R1 found two Warnings: stored workspace comparison skipped
D6a's resolution rule, and new/switch lacked private admitted seams needed by
D6b3's fixed write set. It also identified bounded clear-failure/lifecycle
coverage and stale SDK limit prose. The repair resolves stored cwd values, adds
shared external/admitted entry points, and covers published clear failure plus
one lifecycle. The expanded 113-test gate passed and focused R2 returned CLEAN.
The first full run exposed only two stale candidate-abort entries in the
exhaustive extension-generation call inventory; the focused repair passed 114
