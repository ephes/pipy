...
| Compaction | `test_native_coding_session_resume_compact.py::test_repeated_compaction_persists_each_cut_and_reopens_without_summary_groups` and `test_native_coding_session_tree.py` | Whole-group semantic-summary requests, cancellation, stale-result refusal, reopen after each cut, and destination-summary replacement are covered by synthetic providers, including `test_native_semantic_compaction.py`. Live semantic quality remains unverified. |
| Context replacement during a run | `test_native_coding_effects.py::test_retained_model_control_stops_stale_coding_run_publication` | A retained model control may replace the context; the stale run raises and closes the session lifetime before publishing stale history, tree entries, or usage. No live recovery is provided. |
